...
public class SVNSequenceDeltaGeneratorTest extends TestCase {
	...
	private void test(String workFile, String workEol, String baseFile, String baseEol, int memoryThreshold, double searchDepthExponent) throws SVNException, IOException {
		...
		final ByteArrayOutputStream testData;
		try {
			final ByteArrayInputStream baseData = new ByteArrayInputStream(baseFile.getBytes());
			testData = new ByteArrayOutputStream();

			final SVNDiffWindowApplyBaton baton = SVNDiffWindowApplyBaton.create(baseData, testData, null);
			for (int index = 0; index < consumer.getWindows().size(); index++) {
				...
				window.apply(baton, new ByteArrayInputStream(stream.toByteArray()));
			}
		}
		finally {
