...
function publish_examples($url){
	...
	$examplesListFrameFile = $_SERVER["DOCUMENT_ROOT"] . "/svn/examples/ExamplesList.html";
	...
	for($k = 0; $k < count($examplesPath); $k++) {
		...
		for($l = 0; $l < count($result[$examplesPath[$k]][$examplesPath[$k]]); $l++){
			fwrite($fhandle, "<br><span class=\"tree-icon\" style=\"font-size: 100%; width: 25em;\">&nbsp;&nbsp;&nbsp;...<a style=\"text-decoration: none;\" href=\"display_example.php?fileURL=".$examplesPath[$k].$result[$examplesPath[$k]][$examplesPath[$k]][$l]."\" TARGET=\"ExamplesDisplay\"><img src=\"text.gif\" border=\"0\">".$result[$examplesPath[$k]][$examplesPath[$k]][$l]."</a></span>");
		}
		fwrite($fhandle, "</div>");
