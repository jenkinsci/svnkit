 ...
 */
public class QSequenceLineReader {
    ...
    public QSequenceLine[] read(InputStream rawStream) throws IOException {
        ...
        try {
            ...
            for (;;) {
                ...
                case -1:
                    if (length > 0) {
                        ...
                        lines.add(new QSequenceLine(from, from + bytes.length - 1, bytes));
                        lastLength = length;
                    }
                    from = from + length;
                    length = 0;
                ...
                if (ch == -1) {
                    ...
                    if (myCustomEolBytes != null && lastLength < buffer.length && lastLength >= 0) {
                        if (buffer[lastLength] == '\r' || buffer[lastLength] == '\n') {
                            lines.add(new QSequenceLine(from, from - 1, new byte[0]));
                        }
                    }
                    break;
                }
            ...
            return (QSequenceLine[]) lines.toArray(new QSequenceLine[lines.size()]);
        } finally {
            stream.close();
        }
    }
