...
import org.tmatesoft.svn.core.internal.wc17.db.SVNWCDb.CopyFromInfo;
...
public class SVNWCDb implements ISVNWCDb {
    ...
    public void elideCopyFromTemp(File localAbspath) throws SVNException {
        assert (isAbsolute(localAbspath));
        DirParsedInfo parseDir = parseDir(localAbspath, Mode.ReadWrite);
        SVNWCDbDir pdh = parseDir.wcDbDir;
        File localRelpath = parseDir.localRelPath;
        verifyDirUsable(pdh);
        elideCopyFrom(pdh, localRelpath);
    }

    private void elideCopyFrom(SVNWCDbDir pdh, File localRelpath) throws SVNException {
        SVNSqlJetStatement stmt;
        boolean haveRow;
        long originalReposId;
        File originalReposRelpath;
        long originalRevision;
        File parentLocalRelpath;
        String name;
        long parentReposId;
        File parentReposRelpath;
        long parentRevision;
        File impliedRelpath;
        stmt = pdh.getWCRoot().getSDb().getStatement(SVNWCDbStatements.SELECT_WORKING_NODE);
        try {
            stmt.bindf("is", pdh.getWCRoot().getWcId(), localRelpath);
            haveRow = stmt.next();
            if (!haveRow) {
            ...
            if (stmt.isColumnNull(SVNWCDbSchema.NODES__Fields.repos_id)) {
                return;
            }
            originalReposId = stmt.getColumnLong(SVNWCDbSchema.NODES__Fields.repos_id);
            originalReposRelpath = SVNFileUtil.createFilePath(stmt.getColumnString(SVNWCDbSchema.NODES__Fields.repos_path));
            originalRevision = stmt.getColumnLong(SVNWCDbSchema.NODES__Fields.revision);
        } finally {
            stmt.reset();
        }
        parentLocalRelpath = SVNFileUtil.getFileDir(localRelpath);
        name = SVNFileUtil.getFileName(localRelpath);
        CopyFromInfo copyFrom = getCopyFrom(pdh, parentLocalRelpath);
        parentReposId = copyFrom.reposId;
        parentReposRelpath = copyFrom.relpath;
        parentRevision = copyFrom.revnum;
        if (parentRevision == INVALID_REVNUM) {
            return;
        }
        if (originalRevision != parentRevision) {
            return;
        }
        if (SVNWCUtils.getPathAsChild(parentReposRelpath, originalReposRelpath) == null) {
            return;
        }
        impliedRelpath = SVNFileUtil.createFilePath(parentReposRelpath, name);
        if (!impliedRelpath.equals(originalReposRelpath)) {
            return;
        }
        if (originalReposId != parentReposId) {
            return;
        }
        stmt = pdh.getWCRoot().getSDb().getStatement(SVNWCDbStatements.UPDATE_COPYFROM_TO_INHERIT);
        stmt.bindf("is", pdh.getWCRoot().getWcId(), localRelpath);
        stmt.done();
    }

    public static class CopyFromInfo {
    ...
    private CopyFromInfo getCopyFrom(SVNWCDbDir pdh, File localRelpath) throws SVNException {
        CopyFromInfo copyFrom = new CopyFromInfo();
        SVNSqlJetStatement stmt = pdh.getWCRoot().getSDb().getStatement(SVNWCDbStatements.SELECT_WORKING_NODE);
        try {
            stmt.bindf("is", pdh.getWCRoot().getWcId(), localRelpath);
            boolean haveRow = stmt.next();
            if (!haveRow) {
                copyFrom.reposId = 0;
                copyFrom.relpath = null;
                copyFrom.revnum = INVALID_REVNUM;
                return copyFrom;
            }
            if (stmt.isColumnNull(SVNWCDbSchema.NODES__Fields.repos_id)) {
                File parentRelpath = SVNFileUtil.getFileDir(localRelpath);
                String name = SVNFileUtil.getFileName(localRelpath);
                CopyFromInfo parentCopyFrom = getCopyFrom(pdh, parentRelpath);
                copyFrom.reposId = parentCopyFrom.reposId;
                copyFrom.revnum = parentCopyFrom.revnum;
                File parentCopyfromRelpath = parentCopyFrom.relpath;
                if (parentCopyfromRelpath != null) {
                    copyFrom.relpath = SVNFileUtil.createFilePath(parentCopyfromRelpath, name);
                } else {
                    copyFrom.relpath = null;
                }
                return copyFrom;
            }
            copyFrom.reposId = stmt.getColumnLong(SVNWCDbSchema.NODES__Fields.repos_id);
            copyFrom.relpath = SVNFileUtil.createFilePath(stmt.getColumnString(SVNWCDbSchema.NODES__Fields.repos_path));
            copyFrom.revnum = stmt.getColumnRevnum(SVNWCDbSchema.NODES__Fields.revision);
            return copyFrom;
        } finally {
            stmt.reset();
        }
