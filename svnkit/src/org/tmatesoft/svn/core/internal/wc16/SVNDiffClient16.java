...
public class SVNDiffClient16 extends SVNMergeDriver {
	 ...
	 * @param diffGeneratora diff driver
	 ...
	 * @param diffOptionsdiff options object
	 */
	 ...
	 * @param urla repository location
	 * @param pegRevisiona revision in which <code>url</code> is first looked up
	 * @param rNan old revision
	 * @param rMa new revision
	 * @param recursive<span class="javakeyword">true</span> to descend recursively
	 * @param useAncestryif <span class="javakeyword">true</span> then the paths
	 * ancestry will be noticed while calculating differences,
	 * otherwise not
	 * @param resultthe target {@link java.io.OutputStream} where the differences
	 * will be written to
	 * @throws SVNExceptionif one of the following is true:
	 * <ul>
	 * <li>at least one of <code>rN</code>, <code>rM</code> and
	 * <code>pegRevision</code> is invalid <li>at least one of
	 * <code>rN</code> and <code>rM</code> is a local revision (see{@link SVNRevision#isLocal()}) <li><code>url</code> was not
	 * found in <code>rN</code> <li><code>url</code> was not found
	 * in <code>rM</code>
	 * </ul>
	 * @deprecated use{@link #doDiff(SVNURL,SVNRevision,SVNRevision,SVNRevision,SVNDepth,boolean,OutputStream)}instead
	 */
	public void doDiff(SVNURL url, SVNRevision pegRevision, SVNRevision rN,
			SVNRevision rM, boolean recursive, boolean useAncestry,
			OutputStream result) throws SVNException {
		doDiff(url, pegRevision, rN, rM, SVNDepth
				.getInfinityOrFilesDepth(recursive), useAncestry, result);
	}
	 ...
	 * <p/>
	 * All other options are handled identically to{@link #doDiff(SVNURL,SVNRevision,SVNURL,SVNRevision,SVNDepth,boolean,OutputStream)}.
	 * @param urla repository location
	 * @param pegRevisiona revision in which <code>url</code> is first looked up
	 * @param rNan old revision
	 * @param rMa new revision
	 * @param depthtree depth to process
	 * @param useAncestryif <span class="javakeyword">true</span> then the paths
	 * ancestry will be noticed while calculating differences,
	 * otherwise not
	 * @param resultthe target {@link java.io.OutputStream} where the differences
	 ...
	 * @throws SVNExceptionif one of the following is true:
	 * <ul>
	 * <li>exception with {@link SVNErrorCode#CLIENT_BAD_REVISION}error code - if either of <code>rN</code> and <code>rM</code>
	 * is either {@link SVNRevision#isValid() invalid} or{@link SVNRevision#isLocal() local} <li>exception with{@link SVNErrorCode#FS_NOT_FOUND} error code - <code>url
	 ...
	 */
	public void doDiff(SVNURL url, SVNRevision pegRevision, SVNRevision rN,
			SVNRevision rM, SVNDepth depth, boolean useAncestry,
			OutputStream result) throws SVNException {
	 ...
	 * specified revisions and writes the result to the provided output stream.
	 * <p>
	 * If <code>rM</code> is a local revision (see {@link SVNRevision#isLocal()}), then the Working Copy <code>path</code> is compared with the
	 ...
	 * @param patha Working Copy path
	 * @param pegRevisiona revision in which the repository location of
	 * <code>path</code> is first looked up
	 * @param rNan old revision
	 * @param rMa new revision (or a local one)
	 * @param recursive<span class="javakeyword">true</span> to descend recursively
	 * @param useAncestryif <span class="javakeyword">true</span> then the paths
	 * ancestry will be noticed while calculating differences,
	 * otherwise not
	 ...
	 * </ul>
	 * @deprecated use{@link #doDiff(File,SVNRevision,SVNRevision,SVNRevision,SVNDepth,boolean,OutputStream,Collection)}instead
	 */
	public void doDiff(File path, SVNRevision pegRevision, SVNRevision rN,
			SVNRevision rM, boolean recursive, boolean useAncestry,
			OutputStream result) throws SVNException {
		doDiff(path, pegRevision, rN, rM, SVNDepth
				.getInfinityOrFilesDepth(recursive), useAncestry, result, null);
	}

	/** 
	 * Iterates over the passed in <code>paths</code> calling{@link #doDiff(File,SVNRevision,SVNRevision,SVNRevision,SVNDepth,boolean,OutputStream,Collection)}for each one in the array.
	 * @param pathsarray of working copy paths
	 * @param rNan old revision
	 * @param rMa new revision
	 * @param pegRevisiona revision in which the repository location of
	 ...
	 * @param resultthe target {@link java.io.OutputStream} where the differences
	 * will be written to
	 * @param changeListscollection with changelist names
	 * @throws SVNException
	 * @since 1.2, SVN 1.5
	 */
	public void doDiff(File[] paths, SVNRevision rN, SVNRevision rM,
		...
		for (int i = 0; i < paths.length; i++) {
			...
			try {
				...
				dispatchEvent(SVNEventFactory.createErrorEvent(svne
	 ...
	 * <code>rN</code> and <code>rM</code>.
	 * <p/>
	 * If <code>rM</code> is neither {@link SVNRevision#BASE}, nor{@link SVNRevision#WORKING}, nor {@link SVNRevision#COMMITTED}, and if,
	 ...
	 * <p/>
	 * All other options are handled identically to{@link #doDiff(File,SVNRevision,File,SVNRevision,SVNDepth,boolean,OutputStream,Collection)}.
	 * @param patha Working Copy path
	 * @param pegRevisiona revision in which the repository location of
	 * <code>path</code> is first looked up
	 * @param rNan old revision
	 ...
	 * </code> and <code>rM</code> are either{@link SVNRevision#WORKING} or {@link SVNRevision#BASE} <li>
	...
	public void doDiff(File path, SVNRevision pegRevision, SVNRevision rN,
		...
		if (!(rM == SVNRevision.BASE || rM == SVNRevision.WORKING || rM == SVNRevision.COMMITTED)) {
			if ((rN == SVNRevision.BASE || rN == SVNRevision.WORKING || rN == SVNRevision.COMMITTED)) {
				doDiffURLWC(path, rM, pegRevision, path, rN, true, depth,
						useAncestry, result, changeLists);
			} else {
				doDiffURLURL(null, path, rN, null, path, rM, pegRevision,
						depth, useAncestry, result);
			}
		} else {
			doDiffURLWC(path, rN, pegRevision, path, rM, false, depth,
					useAncestry, result, changeLists);
		}
	}
	 ...
	 * @param url1the first URL to be compared
	 * @param rNa revision of <code>url1</code>
	 * @param url2the second URL to be compared
	 * @param rMa revision of <code>url2</code>
	 * @param recursive<span class="javakeyword">true</span> to descend recursively
	 * @param useAncestryif <span class="javakeyword">true</span> then the paths
	 * ancestry will be noticed while calculating differences,
	 * otherwise not
	 ...
	 * </code>
	 * </ul>
	 * @deprecated use{@link #doDiff(SVNURL,SVNRevision,SVNURL,SVNRevision,SVNDepth,boolean,OutputStream)}instead
	 */
	public void doDiff(SVNURL url1, SVNRevision rN, SVNURL url2,
			SVNRevision rM, boolean recursive, boolean useAncestry,
			OutputStream result) throws SVNException {
		doDiff(url1, rN, url2, rM, SVNDepth.getInfinityOrFilesDepth(recursive),
				useAncestry, result);
	}
	 ...
	 * a non-<span class="javakeyword">null</span>{@link DefaultSVNDiffGenerator#setBasePath(File) base path} provided to
	 ...
	 * Else if {@link SVNDepth#FILES}, behaves as if for{@link SVNDepth#IMMEDIATES} except doesn't diff properties of
	 ...
	 * <p/>
	 * Diffs output will not be generated for binary files, unless{@link ISVNDiffGenerator#isForcedBinaryDiff()} is <span
	 ...
	 * @param url1the first URL to be compared
	 * @param rNa revision of <code>url1</code>
	 * @param url2the second URL to be compared against <code>path1</code>
	 * @param rMa revision of <code>url2</code>
	 * @param depthtree depth to process
	 * @param useAncestryif <span class="javakeyword">true</span> then the paths
	 * ancestry will be noticed while calculating differences,
	 ...
	 * @param resultthe target {@link java.io.OutputStream} where the differences
	 * will be written to
	 * @throws SVNExceptionin the following cases:
	 * <ul>
	 * <li/>exception with {@link SVNErrorCode#CLIENT_BAD_REVISION}error code - if either <code>rN</code> or <code>rM</code> is{@link SVNRevision#isValid() invalid}</ul>
	 * @since 1.2, SVN 1.5
	 */
	public void doDiff(SVNURL url1, SVNRevision rN, SVNURL url2,
			SVNRevision rM, SVNDepth depth, boolean useAncestry,
		...
		doDiffURLURL(url1, null, rN, url2, null, rM, SVNRevision.UNDEFINED,
				depth, useAncestry, result);
	}
	 ...
	 * <p>
	 * If <code>rN</code> is not a local revision (see{@link SVNRevision#isLocal()}), then its repository location URL as it is
	 ...
	 * @param path1a WC path
	 * @param rNa revision of <code>path1</code>
	 * @param url2a repository location URL that is to be compared against
	 ...
	 * </ul>
	 * @deprecated use{@link #doDiff(File,SVNRevision,SVNURL,SVNRevision,SVNDepth,boolean,OutputStream,Collection)}instead
	 */
	public void doDiff(File path1, SVNRevision rN, SVNURL url2, SVNRevision rM,
			boolean recursive, boolean useAncestry, OutputStream result)
			throws SVNException {
		doDiff(path1, rN, url2, rM,
				SVNDepth.getInfinityOrFilesDepth(recursive), useAncestry,
				result, null);
	}
	 ...
	 * <p/>
	 * If both <code>rN</code> is either {@link SVNRevision#WORKING} or{@link SVNRevision#BASE}, then it will be a wc-against-url; otherwise, a
	 ...
	 */
	public void doDiff(File path1, SVNRevision rN, SVNURL url2, SVNRevision rM,
			SVNDepth depth, boolean useAncestry, OutputStream result,
		...
		if (rN == SVNRevision.BASE || rN == SVNRevision.WORKING) {
			doDiffURLWC(url2, rM, SVNRevision.UNDEFINED, path1, rN, true,
					depth, useAncestry, result, changeLists);
		} else {
			doDiffURLURL(null, path1, rN, url2, null, rM,
					SVNRevision.UNDEFINED, depth, useAncestry, result);
		}
	}
	 ...
	 * provided output stream.
	 * <p>
	 * If <code>rM</code> is not a local revision (see{@link SVNRevision#isLocal()}), then its repository location URL as it is
	 ...
	 * @param url1a repository location URL
	 * @param rNa revision of <code>url1</code>
	 * @param path2a WC path that is to be compared against <code>url1</code>
	 * @param rMa revision of <code>path2</code>
	 * @param recursive<span class="javakeyword">true</span> to descend recursively
	 * @param useAncestryif <span class="javakeyword">true</span> then the paths
	 * ancestry will be noticed while calculating differences,
	 * otherwise not
	 ...
	 * </ul>
	 * @deprecated use{@link #doDiff(SVNURL,SVNRevision,File,SVNRevision,SVNDepth,boolean,OutputStream,Collection)}instead
	 */
	public void doDiff(SVNURL url1, SVNRevision rN, File path2, SVNRevision rM,
			boolean recursive, boolean useAncestry, OutputStream result)
			throws SVNException {
		doDiff(url1, rN, path2, rM,
				SVNDepth.getInfinityOrFilesDepth(recursive), useAncestry,
				result, null);
	}
	 ...
	 * side of the diff represents working copy data.
	 * <p/>
	 * If both <code>rM</code> is either {@link SVNRevision#WORKING} or{@link SVNRevision#BASE}, then it will be a url-against-wc; otherwise, a
	 * url-against-url diff.
	 * @param url1a repository location URL
	 * @param rNa revision of <code>url1</code>
	 * @param path2a WC path that is to be compared against <code>url1</code>
	...
	public void doDiff(SVNURL url1, SVNRevision rN, File path2, SVNRevision rM,
		...
		if (rM == SVNRevision.BASE || rM == SVNRevision.WORKING) {
			doDiffURLWC(url1, rN, SVNRevision.UNDEFINED, path2, rM, false,
					depth, useAncestry, result, changeLists);
		} else {
			doDiffURLURL(url1, null, rN, null, path2, rM,
					SVNRevision.UNDEFINED, depth, useAncestry, result);
		}
	}
	 ...
	 * stream.
	 * <p>
	 * If both <code>rN</code> and <code>rM</code> are local revisions (see{@link SVNRevision#isLocal()}), then a Working Copy <code>path2</code> is
	 ...
	 * @param path1a WC path
	 * @param rNa revision of <code>path1</code>
	 * @param path2a WC path that is to be compared against <code>path1</code>
	 * @param rMa revision of <code>path2</code>
	 * @param recursive<span class="javakeyword">true</span> to descend recursively
	 * @param useAncestryif <span class="javakeyword">true</span> then the paths
	 * ancestry will be noticed while calculating differences,
	 ...
	 * </code> is not {@link SVNRevision#WORKING}</ul>
	 * @deprecated use{@link #doDiff(File,SVNRevision,File,SVNRevision,SVNDepth,boolean,OutputStream,Collection)}instead
	 */
	public void doDiff(File path1, SVNRevision rN, File path2, SVNRevision rM,
			boolean recursive, boolean useAncestry, OutputStream result)
			throws SVNException {
		doDiff(path1, rN, path2, rM, SVNDepth
				.getInfinityOrFilesDepth(recursive), useAncestry, result, null);
	}
	 ...
	 * side of the diff represents working copy data.
	 * <p/>
	 * If both <code>rN</code> and <code>rM</code> are either{@link SVNRevision#WORKING} or {@link SVNRevision#BASE}, then it will be
	...
	public void doDiff(File path1, SVNRevision rN, File path2, SVNRevision rM,
		...
		boolean isPath1Local = rN == SVNRevision.WORKING
				|| rN == SVNRevision.BASE;
		boolean isPath2Local = rM == SVNRevision.WORKING
				|| rM == SVNRevision.BASE;
		getDiffGenerator().init(path1.getAbsolutePath(),
		...
		if (isPath1Local && isPath2Local) {
			...
			doDiffURLWC(path2, rM, SVNRevision.UNDEFINED, path1, rN, true,
					depth, useAncestry, result, changeLists);
		} else if (isPath2Local) {
			doDiffURLWC(path1, rN, SVNRevision.UNDEFINED, path2, rM, false,
					depth, useAncestry, result, changeLists);
		} else {
			doDiffURLURL(null, path1, rN, null, path2, rM,
					SVNRevision.UNDEFINED, depth, useAncestry, result);
		}
	}
	 ...
	 * @param path1the path of a left-hand item to diff
	 * @param rNa revision of <code>path1</code>
	 * @param path2the path of a right-hand item to diff
	 * @param rMa revision of <code>path2</code>
	 * @param recursivecontrols whether operation must recurse or not
	 * @param useAncestryif <span class="javakeyword">true</span> then the paths
	 * ancestry will be noticed while calculating differences,
	 * otherwise not
	 * @param handlera diff status handler
	 * @throws SVNException
	 * @since 1.1, new in Subversion 1.4
	 * @deprecated use{@link #doDiffStatus(File,SVNRevision,File,SVNRevision,SVNDepth,boolean,ISVNDiffStatusHandler)}instead
	 */
	public void doDiffStatus(File path1, SVNRevision rN, File path2,
			SVNRevision rM, boolean recursive, boolean useAncestry,
			ISVNDiffStatusHandler handler) throws SVNException {
		doDiffStatus(path1, rN, path2, rM, SVNDepth
				.getInfinityOrFilesDepth(recursive), useAncestry, handler);
	}
	 ...
	 * behaves identically to{@link #doDiffStatus(File,SVNRevision,File,SVNRevision,SVNDepth,boolean,ISVNDiffStatusHandler)}, using <code>path</code> for both of that method's <code>path1</code>
	 ...
	 * for{@link #doDiffStatus(File,SVNRevision,File,SVNRevision,SVNDepth,boolean,ISVNDiffStatusHandler)}.
	 * <p/>
	 * Calls <code>handler</code> for each difference with an{@link SVNDiffStatus} object describing the difference.
	 * <p/>
	 * See{@link #doDiff(File,SVNRevision,SVNRevision,SVNRevision,SVNDepth,boolean,OutputStream,Collection)}for a description of the other parameters.
	 * @param pathworking copy path
	 * @param rNleft-hand revision
	 * @param rMright-hand revision
	 * @param pegRevisiona revision in which the repository location of
	 * <code>path</code> is first looked up
	 * @param depthtree depth to process
	 * @param useAncestryif <span class="javakeyword">true</span> then the paths
	 ...
	 * @since 1.2, SVN 1.5
	 */
	public void doDiffStatus(File path, SVNRevision rN, SVNRevision rM,
			SVNRevision pegRevision, SVNDepth depth, boolean useAncestry,
			ISVNDiffStatusHandler handler) throws SVNException {
		if (handler == null) {
			return;
		...
		doDiffURLURL(null, path, rN, null, path, rM, pegRevision, depth,
				useAncestry, handler);
	}
	 ...
	 * Calls <code>handler</code> for each difference with an{@link SVNDiffStatus} object describing the difference.
	 * <p/>
	 * See{@link #doDiff(File,SVNRevision,File,SVNRevision,SVNDepth,boolean,OutputStream,Collection)}for a description of the other parameters.
	 * @param path1the path of a left-hand item to diff
	 * @param rNa revision of <code>path1</code>
	 * @param path2the path of a right-hand item to diff
	 * @param rMa revision of <code>path2</code>
	 ...
	 * @throws SVNExceptionin the following cases:
	 * <ul>
	 * <li/>exception with {@link SVNErrorCode#CLIENT_BAD_REVISION}error code - if either <code>rN</code> or <code>rM</code> is{@link SVNRevision#isValid() invalid} <li/>exception with{@link SVNErrorCode#UNSUPPORTED_FEATURE} error code - if
	 * either of <code>rM</code> or </code>rN</code> is either{@link SVNRevision#WORKING} or {@link SVNRevision#BASE}</ul>
	 * @since 1.2, SVN 1.5
	 */
	public void doDiffStatus(File path1, SVNRevision rN, File path2,
			SVNRevision rM, SVNDepth depth, boolean useAncestry,
		...
		doDiffURLURL(null, path1, rN, null, path2, rM, SVNRevision.UNDEFINED,
				depth, useAncestry, handler);
	}
	 ...
	 * @param path1the path of a left-hand item to diff
	 * @param rNa revision of <code>path1</code>
	 * @param url2the url of a right-hand item to diff
	 * @param rMa revision of <code>url2</code>
	 * @param recursivecontrols whether operation must recurse or not
	 * @param useAncestryif <span class="javakeyword">true</span> then the paths
	 * ancestry will be noticed while calculating differences,
	 ...
	 * @throws SVNException
	 * @since 1.1, new in Subversion 1.4
	 * @deprecated use{@link #doDiffStatus(File,SVNRevision,SVNURL,SVNRevision,SVNDepth,boolean,ISVNDiffStatusHandler)}instead
	 */
	public void doDiffStatus(File path1, SVNRevision rN, SVNURL url2,
			SVNRevision rM, boolean recursive, boolean useAncestry,
			ISVNDiffStatusHandler handler) throws SVNException {
		doDiffStatus(path1, rN, url2, rM, SVNDepth
				.getInfinityOrFilesDepth(recursive), useAncestry, handler);
	}
	 ...
	 * Calls <code>handler</code> for each difference with an{@link SVNDiffStatus} object describing the difference.
	 * <p/>
	 * See{@link #doDiff(File,SVNRevision,SVNURL,SVNRevision,SVNDepth,boolean,OutputStream,Collection)}for a description of the other parameters.
	 * @param path1the path of a left-hand item to diff
	 * @param rNa revision of <code>path1</code>
	 * @param url2repository url as a right-hand item
	 * @param rMa revision of <code>url2</code>
	 * @param depthtree depth to process
	 * @param useAncestryif <span class="javakeyword">true</span> then the paths
	 * ancestry will be noticed while calculating differences,
	 ...
	 * to the provided handler. This method functionality is equivalent to the
	 * 'svn diff --summarize' command.
	 * @param url1the url of a left-hand item to diff
	 * @param rNa revision of <code>url1</code>
	 * @param path2the path of a right-hand item to diff
	 * @param rMa revision of <code>path2</code>
	 * @param recursivecontrols whether operation must recurse or not
	 ...
	 * @throws SVNException
	 * @since 1.1, new in Subversion 1.4
	 * @deprecated use{@link #doDiffStatus(SVNURL,SVNRevision,File,SVNRevision,SVNDepth,boolean,ISVNDiffStatusHandler)}instead
	 */
	public void doDiffStatus(SVNURL url1, SVNRevision rN, File path2,
			SVNRevision rM, boolean recursive, boolean useAncestry,
			ISVNDiffStatusHandler handler) throws SVNException {
		doDiffStatus(url1, rN, path2, rM, SVNDepth
				.getInfinityOrFilesDepth(recursive), useAncestry, handler);
	}
	 ...
	 * Calls <code>handler</code> for each difference with an{@link SVNDiffStatus} object describing the difference.
	 * <p/>
	 * See{@link #doDiff(SVNURL,SVNRevision,File,SVNRevision,SVNDepth,boolean,OutputStream,Collection)}for a description of the other parameters.
	 * @param url1repository url as a left-hand item
	 * @param rNa revision of <code>url1</code>
	 * @param path2the path of a right-hand item to diff
	 * @param rMa revision of <code>path2</code>
	 * @param depthtree depth to process
	 ...
	 * @throws SVNException
	 * @since 1.1, new in Subversion 1.4
	 * @deprecated use{@link #doDiffStatus(SVNURL,SVNRevision,SVNURL,SVNRevision,SVNDepth,boolean,ISVNDiffStatusHandler)}instead
	 */
	public void doDiffStatus(SVNURL url1, SVNRevision rN, SVNURL url2,
			SVNRevision rM, boolean recursive, boolean useAncestry,
			ISVNDiffStatusHandler handler) throws SVNException {
		doDiffStatus(url1, rN, url2, rM, SVNDepth
				.getInfinityOrFilesDepth(recursive), useAncestry, handler);
	}
	 ...
	 * <p/>
	 * If <code>pegRevision</code> is {@link SVNRevision#isValid() invalid},
	 * behaves identically to{@link #doDiffStatus(SVNURL,SVNRevision,SVNURL,SVNRevision,SVNDepth,boolean,ISVNDiffStatusHandler)}, using <code>url</code> for both of that method's <code>url1</code> and
	 * <code>url2</code> argments.
	 * <p/>
	 * The method may report false positives if <code>useAncestry</code> is
	 * <span class="javakeyword">false</span>, as described in the documentation
	 * for{@link #doDiffStatus(SVNURL,SVNRevision,SVNURL,SVNRevision,SVNDepth,boolean,ISVNDiffStatusHandler)}.
	 * <p/>
	 * Calls <code>handler</code> for each difference with an{@link SVNDiffStatus} object describing the difference.
	 * <p/>
	 * See{@link #doDiff(SVNURL,SVNRevision,SVNRevision,SVNRevision,SVNDepth,boolean,OutputStream)}for a description of the other parameters.
	 * @param urlrepository url
	 * @param rNleft-hand revision
	 * @param rMright-hand revision
	 * @param pegRevisiona revision in which the repository location of
	 * <code>path</code> is first looked up
	 ...
	 * @since 1.2, SVN 1.5
	 */
	public void doDiffStatus(SVNURL url, SVNRevision rN, SVNRevision rM,
			SVNRevision pegRevision, SVNDepth depth, boolean useAncestry,
			ISVNDiffStatusHandler handler) throws SVNException {
		if (handler == null) {
			return;
	 ...
	 * Calls <code>handler</code> for each difference with an{@link SVNDiffStatus} object describing the difference.
	 * <p/>
	 * See{@link #doDiff(SVNURL,SVNRevision,SVNURL,SVNRevision,SVNDepth,boolean,OutputStream)}for a description of the other parameters.
	 * @param url1the url of a left-hand item to diff
	 * @param rNa revision of <code>url1</code>
	 * @param url2the url of a right-hand item to diff
	 * @param rMa revision of <code>url2</code>
	 ...
	 * @param path1the first source path
	 * @param revision1a revision of <code>path1</code>
	 * @param path2the second source path which URL is to be compared against the
	 * URL of <code>path1</code>
	 * @param revision2a revision of <code>path2</code>
	 * @param dstPaththe target path to which the result should be applied
	 * @param recursive<span class="javakeyword">true</span> to descend recursively
	 * @param useAncestryif <span class="javakeyword">true</span> then the paths
	 * ancestry will be noticed while calculating differences,
	 * otherwise not
	 ...
	 * to run
	 * @param dryRunif <span class="javakeyword">true</span> then only tries the
	 ...
	 * </ul>
	 * @deprecated use{@link #doMerge(File,SVNRevision,File,SVNRevision,File,SVNDepth,boolean,boolean,boolean,boolean)}instead
	 */
	public void doMerge(File path1, SVNRevision revision1, File path2,
			SVNRevision revision2, File dstPath, boolean recursive,
			boolean useAncestry, boolean force, boolean dryRun)
			throws SVNException {
		doMerge(path1, revision1, path2, revision2, dstPath, SVNDepth
				.getInfinityOrFilesDepth(recursive), useAncestry, force,
				dryRun, false);
	}
	 ...
	 * <p/>{@link #getMergeOptions() merge options} is used to pass arguments to the
	 ...
	 * @param path1left-hand working copy path
	 * @param revision1revision of <code>path1</code>
	 * @param path2right-hand working copy path
	 * @param revision2revision of <code>path2</code>
	 * @param dstPathtarget working copy path
	 * @param depthtree depth to process
	 * @param useAncestryif <span class="javakeyword">true</span> then the paths
	 * ancestry will be noticed while calculating differences,
	 * otherwise not
	 * @param force<span class="javakeyword">true</span> to force the operation
	 * to run
	 * @param dryRunif <span class="javakeyword">true</span> then runs merge
	 * without any file changes
	 * @param recordOnlyif <span class="javakeyword">true</span>, records only the
	 * rusult of merge - mergeinfo data
	 * @throws SVNExceptionin the following cases:
	 * <ul>
	 * <li/>exception with {@link SVNErrorCode#CLIENT_BAD_REVISION}error code - if either <code>revision1</code> or <code>
	 * revision2</code> is {@link SVNRevision#isValid() invalid} 
	 * <li/>exception with {@link SVNErrorCode#ENTRY_MISSING_URL}error code - if failed to retrieve url of either <code>path1
	 * </code> or <code>path2</code>
	 * </ul>
	 * @since 1.2, SVN 1.5
	 */
	public void doMerge(File path1, SVNRevision revision1, File path2,
			SVNRevision revision2, File dstPath, SVNDepth depth,
			boolean useAncestry, boolean force, boolean dryRun,
		...
		if (url1 == null) {
			SVNErrorMessage err = SVNErrorMessage
					.create(SVNErrorCode.ENTRY_MISSING_URL,
							"''{0}'' has no URL", path1);
		...
		runMerge(url1, revision1, url2, revision2, dstPath, depth, dryRun,
	 ...
	 * target path(s). If a path can be successfully merged, the status type
	 * will be {@link SVNStatusType#MERGED} for that path.
	 * @param path1the first source - a WC path
	 * @param revision1a revision of <code>path1</code>
	 * @param url2the second source - a URL that is to be compared against the
	 * URL of <code>path1</code>
	 * @param revision2a revision of <code>url2</code>
	 * @param dstPaththe target path to which the result should be applied
	 * @param recursive<span class="javakeyword">true</span> to descend recursively
	 * @param useAncestryif <span class="javakeyword">true</span> then the paths
	 * ancestry will be noticed while calculating differences,
	 ...
	 * not under version control
	 * </ul>
	 * @deprecated use{@link #doMerge(File,SVNRevision,SVNURL,SVNRevision,File,SVNDepth,boolean,boolean,boolean,boolean)}instead
	 */
	public void doMerge(File path1, SVNRevision revision1, SVNURL url2,
			SVNRevision revision2, File dstPath, boolean recursive,
			boolean useAncestry, boolean force, boolean dryRun)
			throws SVNException {
		doMerge(path1, revision1, url2, revision2, dstPath, SVNDepth
				.getInfinityOrFilesDepth(recursive), useAncestry, force,
				dryRun, false);
	}
	 ...
	 * <p/>
	 * Note: this method requires repository access.
	 * @param path1left-hand item - working copy path
	 * @param revision1revision of <code>path1</code>
	 * @param url2right-hand item - repository url
	 * @param revision2revision of <code>url2</code>
	 * @param dstPathtarget working copy path
	 * @param depthtree depth to process
	 * @param useAncestryif <span class="javakeyword">true</span> then the paths
	 * ancestry will be noticed while calculating differences,
	 ...
	 * <li/>exception with {@link SVNErrorCode#CLIENT_BAD_REVISION}error code - if either <code>revision1</code> or <code>
	 * revision2</code> is {@link SVNRevision#isValid() invalid} 
	 * <li/>exception with {@link SVNErrorCode#ENTRY_MISSING_URL}error code - if failed to retrieve the repository url of
	 * <code>path1</code>
	 * </ul>
	 * @since 1.2, SVN 1.5
	 */
	 ...
	 * target path(s). If a path can be successfully merged, the status type
	 * will be {@link SVNStatusType#MERGED} for that path.
	 * @param url1the first source - a URL
	 * @param revision1a revision of <code>url1</code>
	 * @param path2the second source - a WC path that is to be compared against
	 * <code>url1</code>
	 * @param revision2a revision of <code>path2</code>
	 * @param dstPaththe target path to which the result should be applied
	 * @param recursive<span class="javakeyword">true</span> to descend recursively
	 ...
	 * not under version control
	 * </ul>
	 * @deprecated use{@link #doMerge(SVNURL,SVNRevision,File,SVNRevision,File,SVNDepth,boolean,boolean,boolean,boolean)}instead
	 */
	public void doMerge(SVNURL url1, SVNRevision revision1, File path2,
			SVNRevision revision2, File dstPath, boolean recursive,
			boolean useAncestry, boolean force, boolean dryRun)
			throws SVNException {
		doMerge(url1, revision1, path2, revision2, dstPath, SVNDepth
				.getInfinityOrFilesDepth(recursive), useAncestry, force,
				dryRun, false);
	}
	 ...
	 * <p/>
	 * Note: this method requires repository access.
	 * @param url1left-hand item - repository url
	 * @param revision1revision of <code>url1</code>
	 * @param path2right-hand item - working copy path
	 * @param revision2revision of <code>path2</code>
	 * @param dstPathtarget working copy path
	 * @param depthtree depth to process
	 * @param useAncestryif <span class="javakeyword">true</span> then the paths
	 ...
	 * target path(s). If a path can be successfully merged, the status type
	 * will be {@link SVNStatusType#MERGED} for that path.
	 * @param url1the first source URL
	 * @param revision1a revision of <code>url1</code>
	 * @param url2the second source URL that is to be compared against
	 * <code>url1</code>
	 * @param revision2a revision of <code>url2</code>
	 * @param dstPaththe target path to which the result should be applied
	 * @param recursive<span class="javakeyword">true</span> to descend recursively
	 ...
	 * not under version control
	 * </ul>
	 * @deprecated use{@link #doMerge(SVNURL,SVNRevision,SVNURL,SVNRevision,File,SVNDepth,boolean,boolean,boolean,boolean)}instead
	 */
	public void doMerge(SVNURL url1, SVNRevision revision1, SVNURL url2,
			SVNRevision revision2, File dstPath, boolean recursive,
			boolean useAncestry, boolean force, boolean dryRun)
			throws SVNException {
		doMerge(url1, revision1, url2, revision2, dstPath, SVNDepth
				.getInfinityOrFilesDepth(recursive), useAncestry, force,
				dryRun, false);
	}
	 ...
	 * <p/>
	 * Note: this method requires repository access.
	 * @param url1left-hand repository url
	 * @param revision1revision of <code>url1</code>
	 * @param url2right-hand repository url
	 * @param revision2revision of <code>url2</code>
	 * @param dstPathtarget working copy path
	 * @param depthtree depth to process
	 * @param useAncestryif <span class="javakeyword">true</span> then the paths
	 ...
	 * <ul>
	 * <li/>exception with {@link SVNErrorCode#CLIENT_BAD_REVISION}error code - if either <code>revision1</code> or <code>
	 * revision2</code> is {@link SVNRevision#isValid() invalid}</ul>
	 * @since 1.2, SVN 1.5
	 */
	public void doMerge(SVNURL url1, SVNRevision revision1, SVNURL url2,
			SVNRevision revision2, File dstPath, SVNDepth depth,
	 ...
	 * target path(s). If a path can be successfully merged, the status type
	 * will be {@link SVNStatusType#MERGED} for that path.
	 * @param url1a source URL
	 * @param pegRevisiona revision in which code>url1</code> is first looked up
	 * @param revision1a left-hand revision of <code>url1</code>
	 * @param revision2a right-hand revision of <code>url1</code>
	 * @param dstPaththe target path to which the result should be applied
	 * @param recursive<span class="javakeyword">true</span> to descend recursively
	 * @param useAncestryif <span class="javakeyword">true</span> then the paths
	 * ancestry will be noticed while calculating differences,
	 ...
	 * </ul>
	 * @deprecated use{@link #doMerge(SVNURL,SVNRevision,Collection,File,SVNDepth,boolean,boolean,boolean,boolean)}instead
	 */
	public void doMerge(SVNURL url1, SVNRevision pegRevision,
		...
		doMerge(url1, pegRevision, rangesToMerge, dstPath, SVNDepth
				.getInfinityOrFilesDepth(recursive), useAncestry, force,
				dryRun, false);
	}
	 ...
	 * <p/>
	 * <code>rangesToMerge</code> is a collection of {@link SVNRevisionRange}ranges. These ranges may describe additive and/or subtractive merge
	 ...
	 * @param url1a source URL
	 * @param pegRevisiona revision in which <code>url1</code> is first looked up
	 * @param rangesToMergecollection of revision ranges to merge
	 * @param dstPathtarget working copy path
	 * @param depthtree depth to process
	 * @param useAncestryif <span class="javakeyword">true</span> then the paths
	 * ancestry will be noticed while calculating differences,
	 ...
	 * @throws SVNExceptionin the following cases:
	 * <ul>
	 * <li/>exception with {@link SVNErrorCode#CLIENT_BAD_REVISION}- If any revision in the list of provided ranges is{@link SVNRevision#isValid() invalid}</ul>
	 * @since 1.2, SVN 1.5
	 */
	public void doMerge(SVNURL url1, SVNRevision pegRevision,
			Collection rangesToMerge, File dstPath, SVNDepth depth,
			boolean useAncestry, boolean force, boolean dryRun,
			boolean recordOnly) throws SVNException {
		if (pegRevision == null || !pegRevision.isValid()) {
			pegRevision = SVNRevision.HEAD;
		}
		runPeggedMerge(url1, null, rangesToMerge, pegRevision, dstPath, depth,
				dryRun, force, !useAncestry, recordOnly);
	}
	 ...
	 * target path(s). If a path can be successfully merged, the status type
	 * will be {@link SVNStatusType#MERGED} for that path.
	 * @param path1a source WC path
	 * @param pegRevisiona revision in which the repository location of
	 * <code>path1</code> is first looked up
	 * @param revision1a left-hand revision of <code>path1</code>
	 * @param revision2a right-hand revision of <code>path1</code>
	 * @param dstPaththe target path to which the result should be applied
	 * @param recursive<span class="javakeyword">true</span> to descend recursively
	 * @param useAncestryif <span class="javakeyword">true</span> then the paths
	 * ancestry will be noticed while calculating differences,
	 ...
	 * version control
	 * </ul>
	 * @deprecated use{@link #doMerge(File,SVNRevision,Collection,File,SVNDepth,boolean,boolean,boolean,boolean)}instead
	 */
	public void doMerge(File path1, SVNRevision pegRevision,
			SVNRevision revision1, SVNRevision revision2, File dstPath,
			boolean recursive, boolean useAncestry, boolean force,
			boolean dryRun) throws SVNException {
		SVNRevisionRange range = new SVNRevisionRange(revision1, revision2);
		List rangesToMerge = new LinkedList();
		rangesToMerge.add(range);
		doMerge(path1, pegRevision, rangesToMerge, dstPath, SVNDepth
				.getInfinityOrFilesDepth(recursive), useAncestry, force,
				dryRun, false);
	}
	 ...
	 * <p/>
	 * Note: this method requires repository access.
	 * @param path1working copy path
	 * @param pegRevisiona revision in which <code>path1</code> is first looked up
	 * @param rangesToMergecollection of revision ranges to merge
	 * @param dstPathtarget working copy path
	 * @param depthtree depth to process
	 * @param useAncestryif <span class="javakeyword">true</span> then the paths
	...
	public void doMerge(File path1, SVNRevision pegRevision,
		...
		runPeggedMerge(null, path1, rangesToMerge, pegRevision, dstPath, depth,
				dryRun, force, !useAncestry, recordOnly);
	}
	 ...
	 * <p/>
	 * <code>dstPath</code> must be a single-revision, {@link SVNDepth#INFINITY}, pristine, unswitched working copy -- in other words, it must reflect a
	 ...
	 * <p/>
	 * If <code>pegRevision</code> is <span class="javakeyword">null</span> or{@link SVNRevision#isValid() invalid}, then it defaults to{@link SVNRevision#WORKING}.
	 * <p/>
	 * Note: this method requires repository access.
	 * @param srcPathworking copy path
	 * @param pegRevisiona revision in which <code>srcPath</code> is first looked up
	 * @param dstPathtarget working copy path
	 * @param dryRunif <span class="javakeyword">true</span> then only tries the
	 * operation to run (to find out if a file can be merged
	 * successfully)
	 ...
	 * @since 1.2, SVN 1.5
	 */
	public void doMergeReIntegrate(File srcPath, SVNRevision pegRevision,
			File dstPath, boolean dryRun) throws SVNException {
		if (pegRevision == null || !pegRevision.isValid()) {
			pegRevision = SVNRevision.WORKING;
		}
	 ...
	 * difference is excatly the branch changes which will go back to trunk).
	 * <p/>
	 * All other options are handled identically to{@link #doMerge(SVNURL,SVNRevision,SVNURL,SVNRevision,File,SVNDepth,boolean,boolean,boolean,boolean)}. The depth of the merge is always {@link SVNDepth#INFINITY}.
	 * <p/>
	 * If <code>pegRevision</code> is <span class="javakeyword">null</span> or{@link SVNRevision#isValid() invalid}, then it defaults to{@link SVNRevision#HEAD}.
	 * <p/>
	 * Note: this method requires repository access.
	 * @param srcURLrepository url
	 * @param pegRevisiona revision in which <code>srcURL</code> is first looked up
	 * @param dstPathtarget working copy path
	 * @param dryRunif <span class="javakeyword">true</span> then only tries the
	 * operation to run (to find out if a file can be merged
	 * successfully)
	 ...
	 * @since 1.2, SVN 1.5
	 */
	public void doMergeReIntegrate(SVNURL srcURL, SVNRevision pegRevision,
			File dstPath, boolean dryRun) throws SVNException {
		if (pegRevision == null || !pegRevision.isValid()) {
			pegRevision = SVNRevision.HEAD;
		}
	 ...
	 * the same as for{@link SVNLogClient#doLog(File[],SVNRevision,SVNRevision,SVNRevision,boolean,boolean,boolean,long,String[],ISVNLogEntryHandler)}.
	 * <p/>
	 * Note: this routine requires repository access.
	 * @param pathworking copy path (merge target)
	 * @param pegRevisiona revision in which <code>path</code> is first looked up
	 * @param mergeSrcURLmerge source repository url
	 * @param srcPegRevisiona revision in which <code>mergeSrcURL</code> is first looked
	 * up
	 * @param discoverChangedPaths<span class="javakeyword">true</span> to report of all changed
	 * paths for every revision being processed (those paths will be
	 * available by calling{@link org.tmatesoft.svn.core.SVNLogEntry#getChangedPaths()})
	 * @param revisionPropertiesnames of revision properties to retrieve
	 * @param handlerthe caller's log entry handler
	 * @throws SVNExceptionin the following cases:
	 * <ul>
	 * <li/>exception with {@link SVNErrorCode#UNSUPPORTED_FEATURE}error code - if the server doesn't support retrieval of
	 * mergeinfo
	 * </ul>
	 * @since 1.2, SVN 1.5
	 */
	public void doGetLogMergedMergeInfo(File path, SVNRevision pegRevision,
			SVNURL mergeSrcURL, SVNRevision srcPegRevision,
			boolean discoverChangedPaths, String[] revisionProperties,
			ISVNLogEntryHandler handler) throws SVNException {
		getLogMergedMergeInfoImpl(path, null, pegRevision, mergeSrcURL, null,
				srcPegRevision, discoverChangedPaths, revisionProperties,
				handler);
	}
	 ...
	 * <p/>
	 * Note: this routine requires repository access.
	 * @param urlrepository url (merge target)
	 * @param pegRevisiona revision in which <code>url</code> is first looked up
	 * @param mergeSrcURLmerge source repository url
	 * @param srcPegRevisiona revision in which <code>mergeSrcURL</code> is first looked
	 * up
	 ...
	 * @since 1.2, SVN 1.5
	 */
	public void doGetLogMergedMergeInfo(SVNURL url, SVNRevision pegRevision,
			SVNURL mergeSrcURL, SVNRevision srcPegRevision,
			boolean discoverChangedPaths, String[] revisionProperties,
			ISVNLogEntryHandler handler) throws SVNException {
		getLogMergedMergeInfoImpl(null, url, pegRevision, mergeSrcURL, null,
				srcPegRevision, discoverChangedPaths, revisionProperties,
				handler);
	}
	 ...
	 * @param pathworking copy path (merge target)
	 * @param pegRevisiona revision in which <code>path</code> is first looked up
	 * @param mergeSrcPathmerge source working copy path
	 * @param srcPegRevisiona revision in which <code>mergeSrcPath</code> is first looked
	 * up
	 * @param discoverChangedPaths<span class="javakeyword">true</span> to report of all changed
	 * paths for every revision being processed (those paths will be
	 * available by calling{@link org.tmatesoft.svn.core.SVNLogEntry#getChangedPaths()})
	 ...
	 */
	public void doGetLogMergedMergeInfo(File path, SVNRevision pegRevision,
			File mergeSrcPath, SVNRevision srcPegRevision,
			boolean discoverChangedPaths, String[] revisionProperties,
			ISVNLogEntryHandler handler) throws SVNException {
		getLogMergedMergeInfoImpl(path, null, pegRevision, null, mergeSrcPath,
				srcPegRevision, discoverChangedPaths, revisionProperties,
				handler);
	}
	...
	public void doGetLogMergedMergeInfo(SVNURL url, SVNRevision pegRevision,
			...
			boolean discoverChangedPaths, String[] revisionProperties,
			ISVNLogEntryHandler handler) throws SVNException {
		getLogMergedMergeInfoImpl(null, url, pegRevision, null, mergeSrcPath,
				srcPegRevision, discoverChangedPaths, revisionProperties,
				handler);
	}
	...
	public void doGetLogEligibleMergeInfo(File path, SVNRevision pegRevision,
			...
			boolean discoverChangedPaths, String[] revisionProperties,
			ISVNLogEntryHandler handler) throws SVNException {
		getLogEligibleMergeInfoImpl(path, null, pegRevision, mergeSrcURL, null,
				srcPegRevision, discoverChangedPaths, revisionProperties,
				handler);
	}
	...
	public void doGetLogEligibleMergeInfo(SVNURL url, SVNRevision pegRevision,
			...
			boolean discoverChangedPaths, String[] revisionProperties,
			ISVNLogEntryHandler handler) throws SVNException {
		getLogEligibleMergeInfoImpl(null, url, pegRevision, mergeSrcURL, null,
				srcPegRevision, discoverChangedPaths, revisionProperties,
				handler);
	}
	...
	public void doGetLogEligibleMergeInfo(File path, SVNRevision pegRevision,
			...
			boolean discoverChangedPaths, String[] revisionProperties,
			ISVNLogEntryHandler handler) throws SVNException {
		getLogEligibleMergeInfoImpl(path, null, pegRevision, null,
				mergeSrcPath, srcPegRevision, discoverChangedPaths,
	...
	public void doGetLogEligibleMergeInfo(SVNURL url, SVNRevision pegRevision,
			...
			boolean discoverChangedPaths, String[] revisionProperties,
			ISVNLogEntryHandler handler) throws SVNException {
		getLogEligibleMergeInfoImpl(null, url, pegRevision, null, mergeSrcPath,
				srcPegRevision, discoverChangedPaths, revisionProperties,
				handler);
	}

	/** 
	 * Returns mergeinfo as a <code>Map</code> with merge source URLs (as{@link SVNURL}) mapped to range lists ({@link SVNMergeRangeList}). Range
	 * lists are objects containing arrays of {@link SVNMergeRange ranges}describing the ranges which have been merged into <code>path</code> as of
	 ...
	 * @since 1.2, SVN 1.5
	 */
	public Map doGetMergedMergeInfo(File path, SVNRevision pegRevision)
			throws SVNException {
		...
		if (mergeInfo != null) {
			...
			for (Iterator paths = mergeInfo.keySet().iterator(); paths
					.hasNext();) {
				String mergeSrcPath = (String) paths.next();
				SVNMergeRangeList rangeList = (SVNMergeRangeList) mergeInfo
						.get(mergeSrcPath);
	...
	/** 
	 * Returns mergeinfo as a <code>Map</code> with merge source URLs (as{@link SVNURL}) mapped to range lists ({@link SVNMergeRangeList}). Range
	 * lists are objects containing arrays of {@link SVNMergeRange ranges}describing the ranges which have been merged into <code>url</code> as of
	 * <code>pegRevision</code>. If there is no mergeinfo, returns <span
	 * class="javakeyword">null</span>.
	 * <p/>
	 * Note: unlike most APIs which deal with mergeinfo, this one returns data
	 ...
	 * @since 1.2, SVN 1.5
	 */
	public Map doGetMergedMergeInfo(SVNURL url, SVNRevision pegRevision)
			throws SVNException {
		SVNURL reposRoot[] = new SVNURL[1];
	 ...
	 * @since 1.2, SVN 1.5
	 */
	public Collection doSuggestMergeSources(File path, SVNRevision pegRevision)
			throws SVNException {
		...
		if (copyFromPath != null) {
			String relCopyFromPath = copyFromPath.startsWith("/") ? copyFromPath
					.substring(1)
					: copyFromPath;
		...
		if (mergeInfo != null) {
			for (Iterator mergeSrcURLs = mergeInfo.keySet().iterator(); mergeSrcURLs
					.hasNext();) {
	 ...
	 * @since 1.2, SVN 1.5
	 */
	public Collection doSuggestMergeSources(SVNURL url, SVNRevision pegRevision)
			throws SVNException {
		LinkedList suggestions = new LinkedList();
	...
	private void doDiffURLWC(SVNURL url1, SVNRevision revision1,
	...
	private void doDiffURLWC(File path1, SVNRevision revision1,
			SVNRevision pegRevision, File path2, SVNRevision revision2,
			boolean reverse, SVNDepth depth, boolean useAncestry,
			OutputStream result, Collection changeLists) throws SVNException {
		SVNWCAccess wcAccess = createWCAccess();
	...
	private void doDiffWCWC(File path1, SVNRevision revision1, File path2,
	...
	private void doDiffURLURL(SVNURL url1, File path1, SVNRevision revision1,
		...
		if (pegRevision.isValid()) {
			SVNRepositoryLocation[] locations = getLocations(url2, path2, null,
			...
			if (kind1 == SVNNodeKind.NONE) {
				SVNErrorMessage err = SVNErrorMessage
						.create(
								...
								"''{0}'' was not found in the repository at revision {1}",
		...
		if (kind1 == SVNNodeKind.FILE || kind2 == SVNNodeKind.FILE) {
			...
			url1 = SVNURL.parseURIEncoded(SVNPathUtil.removeTail(url1
					.toString()));
		...
		try {
			SVNDiffCallback callback = new SVNDiffCallback(null,
					getDiffGenerator(), rev1, rev2, result);
			callback.setBasePath(basePath);
			editor = new SVNRemoteDiffEditor(null, null, callback, repository2,
					rev1, rev2, false, null, this);
			...
			repository1.diff(url2, rev2, rev1, target1, !useAncestry, depth,
					true, reporter, SVNCancellableEditor.newInstance(editor,
							this, getDebugLog()));
		} finally {
			if (editor != null) {
				editor.cleanup();
	...
	private void doDiffURLURL(SVNURL url1, File path1, SVNRevision revision1,
		...
		try {
			SVNDiffStatusEditor editor = new SVNDiffStatusEditor(basePath,
					target1, repository2, rev1, handler);
			ISVNReporterBaton reporter = new ISVNReporterBaton() {
				public void report(ISVNReporter reporter) throws SVNException {
					reporter.setPath("", null, rev1, SVNDepth.INFINITY, false);
					reporter.finishReport();
			...
			repository1.diff(url2, rev2, rev1, target1, !useAncestry, depth,
					false, reporter, SVNCancellableEditor.newInstance(editor,
							this, getDebugLog()));
		} finally {
			if (tmpFile != null) {
	...
	private Object[] getLocationFromPathAndRevision(File path, SVNURL url,
	...
	private void getLogMergedMergeInfoImpl(File path, SVNURL url,
			SVNRevision pegRevision, SVNURL mergeSrcURL, File mergeSrcPath,
			SVNRevision srcPegRevision, boolean discoverChangedPaths,
			String[] revisionProperties, ISVNLogEntryHandler handler)
			throws SVNException {
		SVNURL reposRoot[] = new SVNURL[1];
		Object[] location = getLocationFromPathAndRevision(mergeSrcPath,
				mergeSrcURL, srcPegRevision);
		SVNURL realMergeSrcURL = (SVNURL) location[0];
		SVNRevision realSrcPegRevision = (SVNRevision) location[1];
		Map targetMergeInfo = path != null ? getMergeInfo(path, pegRevision,
				reposRoot) : getMergeInfo(url, pegRevision, reposRoot);
		if (targetMergeInfo == null) {
			return;
		}
		Map srcHistory = getHistoryAsMergeInfo(realMergeSrcURL, null,
				realSrcPegRevision, SVNRepository.INVALID_REVISION,
				SVNRepository.INVALID_REVISION, null, null);
		Map mergeInfo = SVNMergeInfoUtil.intersectMergeInfo(targetMergeInfo,
				srcHistory, false);
		SVNMergeRangeList rangeList = new SVNMergeRangeList(
				new SVNMergeRange[0]);
		long youngestRev = SVNRepository.INVALID_REVISION;
		String logTarget = null;
		for (Iterator mergeInfoIter = mergeInfo.keySet().iterator(); mergeInfoIter
				.hasNext();) {
			String mergeSrc = (String) mergeInfoIter.next();
			SVNMergeRangeList list = (SVNMergeRangeList) mergeInfo
					.get(mergeSrc);
			SVNMergeRange[] listRanges = list.getRanges();
			SVNMergeRange range = listRanges[listRanges.length - 1];
			if (!SVNRevision.isValidRevisionNumber(youngestRev)
					|| range.getEndRevision() > youngestRev) {
		...
		getLogsForMergeInfoRangeList(reposRoot[0], new String[] { logTarget },
				rangeList, discoverChangedPaths, revisionProperties, handler);
	}

	private void getLogEligibleMergeInfoImpl(File path, SVNURL url,
			SVNRevision pegRevision, SVNURL mergeSrcURL, File mergeSrcPath,
			SVNRevision srcPegRevision, boolean discoverChangedPaths,
			String[] revisionProperties, ISVNLogEntryHandler handler)
			throws SVNException {
		...
		SVNURL realMergeSrcURL = (SVNURL) location[0];
		SVNRevision realSrcPegRevision = (SVNRevision) location[1];
		Map mergeInfo = path != null ? getMergeInfo(path, pegRevision,
				reposRoot) : getMergeInfo(url, pegRevision, reposRoot);
		Map history = getHistoryAsMergeInfo(url, path, pegRevision,
		...
		try {
			...
			sourceHistory = getHistoryAsMergeInfo(realMergeSrcURL, null,
					realSrcPegRevision, SVNRepository.INVALID_REVISION,
					SVNRepository.INVALID_REVISION, repos, null);
		} finally {
			repos.closeSession();
		}
		Map availableMergeInfo = SVNMergeInfoUtil.removeMergeInfo(mergeInfo,
				sourceHistory, false);
		SVNMergeRangeList rangeList = new SVNMergeRangeList(
				new SVNMergeRange[0]);
		long youngestRev = SVNRepository.INVALID_REVISION;
		String logTarget = null;
		for (Iterator availableIter = availableMergeInfo.keySet().iterator(); availableIter
				.hasNext();) {
			String mergeSrc = (String) availableIter.next();
			SVNMergeRangeList availableRangeList = (SVNMergeRangeList) availableMergeInfo
					.get(mergeSrc);
	...
	private void applyPatches(File absPatchPath, File absWCPath,
			boolean dryRun, int stripCount, SVNAdminArea wc)
