...
public class SVNLogClient16 extends SVNBasicDelegate {
	...
	public void doAnnotate(File path, SVNRevision pegRevision,
		...
		doAnnotate(path, pegRevision, startRevision, endRevision, false, false,
				handler, null);
	}
	...
	public void doAnnotate(File path, SVNRevision pegRevision,
			SVNRevision startRevision, SVNRevision endRevision,
			boolean ignoreMimeType, ISVNAnnotateHandler handler)
			throws SVNException {
		doAnnotate(path, pegRevision, startRevision, endRevision,
	 ...
	 * then it defaults to revision 1. If <code>endRevision</code> is <span class="javakeyword">null</span> or {@link SVNRevision#isValid() invalid}, then in defaults to {@link SVNRevision#HEAD}.
	 * <p/>
	...
	public void doAnnotate(File path, SVNRevision pegRevision,
			...
			ISVNAnnotateHandler handler, String inputEncoding)
			throws SVNException {
		...
		if (startRevision == SVNRevision.WORKING
				|| endRevision == SVNRevision.WORKING) {
		...
		SVNRepository repos = createRepository(null, path, null, pegRevision,
				endRevision, null);
		...
		File tmpFile = new File(path.getParentFile(), SVNFileUtil
		...
		doAnnotate(path.getAbsolutePath(), startRev, tmpFile, repos, endRev,
				ignoreMimeType, handler, inputEncoding, includeMergedRevisions);
	}
	 ...
	 */
	public void doAnnotate(SVNURL url, SVNRevision pegRevision,
			SVNRevision startRevision, SVNRevision endRevision,
			ISVNAnnotateHandler handler) throws SVNException {
		doAnnotate(url, pegRevision, startRevision, endRevision, false, false,
				handler, null);
	}
	...
	public void doAnnotate(SVNURL url, SVNRevision pegRevision,
		...
		doAnnotate(url, pegRevision, startRevision, endRevision,
	...
	public void doAnnotate(SVNURL url, SVNRevision pegRevision,
		...
		doAnnotate(repos.getLocation().toDecodedString(), startRev, tmpFile,
				repos, endRev, ignoreMimeType, handler, inputEncoding,
	...
	public void doLog(File[] paths, SVNRevision startRevision,
			SVNRevision endRevision, boolean stopOnCopy,
			boolean discoverChangedPaths, long limit,
			final ISVNLogEntryHandler handler) throws SVNException {
		doLog(paths, startRevision, endRevision, SVNRevision.UNDEFINED,
				stopOnCopy, discoverChangedPaths, false, limit, null, handler);
	}
	 ...
	 */
	public void doLog(File[] paths, SVNRevision startRevision,
			SVNRevision endRevision, SVNRevision pegRevision,
			boolean stopOnCopy, boolean discoverChangedPaths,
			boolean includeMergedRevisions, long limit,
			String[] revisionProperties, final ISVNLogEntryHandler handler)
			throws SVNException {
		Collection revisionRanges = new ArrayList(1);
		revisionRanges.add(new SVNRevisionRange(startRevision, endRevision));
		doLog(paths, revisionRanges, pegRevision, stopOnCopy,
				discoverChangedPaths, includeMergedRevisions, limit,
	 ...
	 * @see #doLog(File[],SVNRevision,SVNRevision,SVNRevision,boolean,boolean,boolean,long,String[],ISVNLogEntryHandler)
	 */
	public void doLog(File[] paths, SVNRevision pegRevision,
			SVNRevision startRevision, SVNRevision endRevision,
			boolean stopOnCopy, boolean discoverChangedPaths, long limit,
			final ISVNLogEntryHandler handler) throws SVNException {
		doLog(paths, startRevision, endRevision, pegRevision, stopOnCopy,
	 ...
	 * <p/>
	 * <code>handler</code> is invoked only on messages whose revisions involved a change to some path in <code>paths</code>. 
	 * <code>pegRevision</code> indicates in which revision <code>paths</code> are valid. If <code>pegRevision</code> is{@link SVNRevision#isValid() invalid}, it defaults to {@link SVNRevision#HEAD}.
	 * <p/>
	 * If <code>limit</code> is non-zero, only invokes <code>handler</code> on the first <code>limit</code> logs.
	 * <p/>
	 * If <code>discoverChangedPaths</code> is set, then the changed paths <code>Map</code> argument
	...
	public void doLog(SVNURL url, String[] paths, SVNRevision pegRevision,
		...
		Collection revisionRanges = new ArrayList(1);
		revisionRanges.add(new SVNRevisionRange(startRevision, endRevision));
		doLog(url, paths, pegRevision, revisionRanges, stopOnCopy,
				discoverChangedPaths, includeMergedRevisions, limit,
				revisionProperties, handler);
	}
	 ...
	 */
	public void doLog(SVNURL url, String[] paths, SVNRevision pegRevision,
			Collection revisionRanges, boolean stopOnCopy,
			boolean discoverChangedPaths, boolean includeMergedRevisions,
			long limit, String[] revisionProperties,
			final ISVNLogEntryHandler handler) throws SVNException {
		SVNRevision sessionRevision = SVNRevision.UNDEFINED;
		...
		for (Iterator revRangesIter = revisionRanges.iterator(); revRangesIter
			...
			if (needsWC(revRange.getStartRevision())
					|| needsWC(revRange.getEndRevision())) {
		...
		SVNRepository repos = sessionRevision.isValid() ? createRepository(url,
				null, null, pegRevision, sessionRevision, null)
				: createRepository(url, null, null, true);
		for (Iterator revRangesIter = editedRevisionRanges.iterator(); revRangesIter
				.hasNext();) {
			checkCancelled();
			SVNRevisionRange revRange = (SVNRevisionRange) revRangesIter.next();
			long startRev = getRevisionNumber(revRange.getStartRevision(),
					repos, null);
			long endRev = getRevisionNumber(revRange.getEndRevision(), repos,
					null);
			repos.log(paths, startRev, endRev, discoverChangedPaths,
					stopOnCopy, limit, includeMergedRevisions,
					revisionProperties, wrappingHandler);
		}
	}
	 ...
	 * information which is packed into an {@link org.tmatesoft.svn.core.SVNDirEntry}object and passed to the <code>handler</code>'s {@link org.tmatesoft.svn.core.ISVNDirEntryHandler#handleDirEntry(SVNDirEntry) handleDirEntry()} method.
	...
	public void doList(File path, SVNRevision pegRevision,
			SVNRevision revision, boolean fetchLocks, boolean recursive,
			ISVNDirEntryHandler handler) throws SVNException {
		doList(path, pegRevision, revision, fetchLocks,
				recursive ? SVNDepth.INFINITY : SVNDepth.IMMEDIATES,
	 ...
	 * If <code>pegRevision</code> is {@link SVNRevision#isValid() invalid}, then it defaults to {@link SVNRevision#WORKING}.
	 * <p/>
	 * Reports directory entries by invoking <code>handler</code>, {@link SVNDirEntry#setRelativePath(String) setting} a relative to <code>path</code> path to the {@link SVNDirEntry} object. The directory entry for <code>path</code> is reported using an empty path. 
	 ...
	 */
	public void doList(File path, SVNRevision pegRevision,
			SVNRevision revision, boolean fetchLocks, SVNDepth depth,
	 ...
	 * info on an entry)
	 * @throws SVNException
	 * @deprecated            use {@link #doList(File,SVNRevision,SVNRevision,boolean,SVNDepth,int,ISVNDirEntryHandler)}instead 
	 */
	public void doList(File path, SVNRevision pegRevision,
			SVNRevision revision, boolean recursive, ISVNDirEntryHandler handler)
			throws SVNException {
		doList(path, pegRevision, revision, false,
				recursive ? SVNDepth.INFINITY : SVNDepth.IMMEDIATES,
				SVNDirEntry.DIRENT_ALL, handler);
	}
	 ...
	 */
	public void doList(SVNURL url, SVNRevision pegRevision,
			SVNRevision revision, boolean fetchLocks, boolean recursive,
			ISVNDirEntryHandler handler) throws SVNException {
		doList(url, pegRevision, revision, fetchLocks,
				recursive ? SVNDepth.INFINITY : SVNDepth.IMMEDIATES,
				SVNDirEntry.DIRENT_ALL, handler);
	}
	 ...
	 * The actual node revision selected is determined by the path as it exists in <code>pegRevision</code>. 
	 * If <code>pegRevision</code> is {@link SVNRevision#isValid() invalid}, then it defaults to {@link SVNRevision#HEAD}.
	 * <p/>
	 * Reports directory entries by invoking <code>handler</code>, {@link SVNDirEntry#setRelativePath(String) setting} a relative to <code>url</code> path to the {@link SVNDirEntry} object. The directory entry for <code>url</code> is reported using an empty path. 
	...
	private void doAnnotate(String path, long startRev, File tmpFile,
			SVNRepository repos, long endRev, boolean ignoreMimeType,
			ISVNAnnotateHandler handler, String inputEncoding,
			boolean includeMergedRevisions) throws SVNException {
		SVNAnnotationGenerator generator = new SVNAnnotationGenerator(path,
				tmpFile, startRev, ignoreMimeType, includeMergedRevisions,
				getDiffOptions(), inputEncoding, handler, this);
		boolean useSpool = handler != null
				&& !handler.getClass().getName().startsWith(
						"org.tmatesoft.svn.");
		...
		try {
			...
			generator.dispose();
			SVNFileUtil.deleteAll(tmpFile, !"text-base".equals(tmpFile
					.getName()), null);
		}
	}

	private void doList(SVNRepository repos, long rev,
			final ISVNDirEntryHandler handler, boolean fetchLocks,
		...
		if (error != null) {
			...
			if (kind != SVNNodeKind.NONE) {
				if (!url.equals(reposRoot)) {
					String name = SVNPathUtil.tail(repos.getLocation()
							.getPath());
					repos.setLocation(repos.getLocation().removePathTail(),
							false);
					Collection dirEntries = repos.getDir("", rev, null,
							entryFields, (Collection) null);
					...
					repos
							.getDir("", rev, props, entryFields,
									(Collection) null);
					SVNProperties revProps = repos.getRevisionProperties(rev,
							null);
					String author = revProps
							.getStringValue(SVNRevisionProperty.AUTHOR);
					String dateStr = revProps
							.getStringValue(SVNRevisionProperty.DATE);
					Date datestamp = null;
					if (dateStr != null) {
						datestamp = SVNDate.parseDateString(dateStr);
					}
					entry = new SVNDirEntry(url, reposRoot, "", kind, 0, !props
							.isEmpty(), rev, datestamp, author);
					entry.setRelativePath("");
				}
			}
		} else if (entry != null) {
			entry.setRelativePath(entry.getKind() == SVNNodeKind.DIR ? ""
					: entry.getName());
		}
		if (entry == null) {
			SVNErrorMessage err = SVNErrorMessage.create(
					SVNErrorCode.FS_NOT_FOUND,
					"URL ''{0}'' non-existent in that revision", url);
			SVNErrorManager.error(err, SVNLogType.WC);
		}
		...
		if (fetchLocks) {
			...
			try {
				...
				if (!(e.getErrorMessage() != null && e.getErrorMessage()
						.getErrorCode() == SVNErrorCode.RA_NOT_IMPLEMENTED)) {
					throw e;
				}
			}
			if (locks != null && locks.length > 0) {
				...
				for (int i = 0; i < locks.length; i++) {
					...
					locksMap.put(root.appendPath(repositoryPath, false),
							locks[i]);
				}
			}
		}
		...
		if (entry.getKind() == SVNNodeKind.DIR
				&& (depth == SVNDepth.FILES || depth == SVNDepth.IMMEDIATES || depth == SVNDepth.INFINITY)) {
			list(repos, "", rev, depth, entryFields, nestedHandler);
		}
	}

	private static void list(SVNRepository repository, String path, long rev,
			SVNDepth depth, int entryFields, ISVNDirEntryHandler handler)
			throws SVNException {
		...
		for (Iterator iterator = entries.iterator(); iterator.hasNext();) {
			...
			if (entry.getKind() == SVNNodeKind.DIR && entry.getDate() != null
					&& depth == SVNDepth.INFINITY) {
