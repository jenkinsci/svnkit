...
public class SVNBasicDelegate implements ISVNEventHandler {
	...
	protected SVNBasicDelegate(final ISVNAuthenticationManager authManager,
			ISVNOptions options) {
	...
	protected SVNBasicDelegate(ISVNRepositoryPool repositoryPool,
			ISVNOptions options) {
	...
	public SVNURL getReposRoot(File path, SVNURL url, SVNRevision pegRevision,
		...
		if (reposRoot == null) {
			...
			try {
				repos = createRepository(url, path, null, pegRevision,
						pegRevision, null);
	...
	protected SVNRepository createRepository(SVNURL url, File path,
	...
	protected long getRevisionNumber(SVNRevision revision,
		...
		if (repository == null
			...
			SVNErrorMessage err = SVNErrorMessage
					.create(SVNErrorCode.CLIENT_RA_ACCESS_REQUIRED);
			SVNErrorManager.error(err, SVNLogType.WC);
		}
		if (revision.getNumber() >= 0) {
			...
			if (latestRevisionNumber != null
					&& latestRevisionNumber.length > 0
					&& SVNRevision
							.isValidRevisionNumber(latestRevisionNumber[0])) {
			...
			if (path == null) {
				SVNErrorMessage err = SVNErrorMessage
						.create(SVNErrorCode.CLIENT_VERSIONED_PATH_REQUIRED);
				SVNErrorManager.error(err, SVNLogType.WC);
			}
			...
			if (entry.getCommittedRevision() < 0) {
				SVNErrorMessage err = SVNErrorMessage.create(
						...
						"Path ''{0}'' has no committed revision", path);
				SVNErrorManager.error(err, SVNLogType.WC);
			}
			return revision == SVNRevision.PREVIOUS ? entry
					.getCommittedRevision() - 1 : entry.getCommittedRevision();
		} else {
			SVNErrorMessage err = SVNErrorMessage.create(
					SVNErrorCode.CLIENT_BAD_REVISION,
					"Unrecognized revision type requested for ''{0}''",
					path != null ? path : (Object) repository.getLocation());
			SVNErrorManager.error(err, SVNLogType.WC);
		}
		return -1;
	}
	...
	protected SVNRepository createRepository(SVNURL url, File path,
		...
		SVNRevision[] resolvedRevisions = resolveRevisions(pegRevision,
				startRevision, url != null, true);
		pegRevision = resolvedRevisions[0];
		startRevision = resolvedRevisions[1];
		SVNRepositoryLocation[] locations = getLocations(url, path, null,
				pegRevision, startRevision, SVNRevision.UNDEFINED);
		url = locations[0].getURL();
		long actualRevision = locations[0].getRevisionNumber();
		SVNRepository repository = createRepository(url, area != null ? area
				.getRoot() : null, area != null ? area.getWCAccess() : null,
				true);
		actualRevision = getRevisionNumber(SVNRevision.create(actualRevision),
				repository, path);
		if (actualRevision < 0) {
	...
	protected SVNRevision[] resolveRevisions(SVNRevision pegRevision,
	...
	protected void elideMergeInfo(SVNWCAccess access, File path,
			SVNEntry entry, File wcElisionLimitPath) throws SVNException {
		if (wcElisionLimitPath == null || !wcElisionLimitPath.equals(path)) {
			...
			if (mergeInfo == null && wcElisionLimitPath == null) {
				mergeInfo = getWCOrRepositoryMergeInfo(path, entry,
						SVNMergeInfoInheritance.NEAREST_ANCESTOR, inherited,
						true, null);
			}
			if (mergeInfo == null && wcElisionLimitPath != null) {
				return;
			}
			SVNMergeInfoUtil.elideMergeInfo(mergeInfo, targetMergeInfo, path,
					null, access);
		}
	}
	 ...
	 */
	protected Map getReposMergeInfo(SVNRepository repository, String path,
			long revision, SVNMergeInfoInheritance inheritance,
			boolean squelchIncapable) throws SVNException {
		Map reposMergeInfo = null;
		try {
			reposMergeInfo = repository.getMergeInfo(new String[] { path },
					revision, inheritance, false);
		} catch (SVNException svne) {
			if (!squelchIncapable
					|| svne.getErrorMessage().getErrorCode() != SVNErrorCode.UNSUPPORTED_FEATURE) {
				throw svne;
			}
		}
		String rootRelativePath = getPathRelativeToRoot(null, repository
				.getLocation(), repository.getRepositoryRoot(false), null,
				repository);
		Map targetMergeInfo = null;
		if (reposMergeInfo != null) {
			SVNMergeInfo mergeInfo = (SVNMergeInfo) reposMergeInfo
	...
	protected Map getWCOrRepositoryMergeInfo(File path, SVNEntry entry,
		...
		Map mergeInfo = null;
		long targetRev[] = { -1 };
		SVNURL url = getEntryLocation(path, entry, targetRev,
				SVNRevision.WORKING);
		long revision = targetRev[0];
		if (!reposOnly) {
			mergeInfo = getWCMergeInfo(path, entry, null, inherit, false,
					indirect);
		}
		if (mergeInfo == null) {
			if (!entry.isScheduledForAddition()) {
				Map fileToProp = SVNPropertiesManager
						.getWorkingCopyPropertyValues(path, entry,
								SVNProperty.MERGE_INFO, SVNDepth.EMPTY, true);
				SVNPropertyValue mergeInfoProp = (SVNPropertyValue) fileToProp
						.get(path);
				if (mergeInfoProp == null) {
					...
					try {
						if (repository == null) {
							repository = createRepository(url, null, null,
									false);
							closeRepository = true;
						}
						repositoryPath = getPathRelativeToSession(url, null,
								repository);
						...
						reposMergeInfo = getReposMergeInfo(repository,
								repositoryPath, revision, inherit, true);
					} finally {
						if (closeRepository) {
	 ...
	 */
	protected Map getWCMergeInfo(File path, SVNEntry entry, File limitPath,
		...
		Map mergeInfo = null;
		if (limitPath != null) {
			limitPath = new File(SVNPathUtil.validateFilePath(limitPath
		...
		try {
			while (true) {
				if (inherit == SVNMergeInfoInheritance.NEAREST_ANCESTOR) {
					...
					wcMergeInfo = SVNPropertiesManager.parseMergeInfo(path,
							entry, base);
				}
				if (SVNWCManager.isEntrySwitched(path, entry)) {
					break;
				}
				path = new File(SVNPathUtil.validateFilePath(path
						.getAbsolutePath())).getAbsoluteFile();
				if (wcMergeInfo == null
		...
		if (walkPath.length() == 0) {
			...
			if (wcMergeInfo != null) {
				mergeInfo = SVNMergeInfoUtil.adjustMergeInfoSourcePaths(null,
						walkPath, wcMergeInfo);
				inherited[0] = true;
			} else {
				mergeInfo = null;
			}
		...
		if (inherited[0]) {
			mergeInfo = SVNMergeInfoUtil.getInheritableMergeInfo(mergeInfo,
	...
	protected long getPathLastChangeRevision(String relPath, long revision,
			SVNRepository repository) throws SVNException {
		final long[] rev = new long[1];
		rev[0] = SVNRepository.INVALID_REVISION;
		repository.log(new String[] { relPath }, 1, revision, false, true, 1,
	...
	protected String getPathRelativeToRoot(File path, SVNURL url,
		...
		if (!absPath.startsWith(reposRootPath)) {
			SVNErrorMessage err = SVNErrorMessage
					.create(
							...
							"URL ''{0}'' is not a child of repository root URL ''{1}''",
							new Object[] { url, reposRootURL });
			SVNErrorManager.error(err, SVNLogType.WC);
		}
	...
	protected String getPathRelativeToSession(SVNURL url, SVNURL sessionURL,
	...
	protected SVNRepositoryLocation[] getLocations(SVNURL url, File path,
			...
			try {
				...
				if (entry.getCopyFromURL() != null
					...
					if (entry.getURL() == null
							|| !entry.getURL().equals(entry.getCopyFromURL())) {
						repository = null;
					}
		...
		if (repository == null) {
			...
			repoPath = SVNPathUtil.getPathAsChild(repository.getLocation()
		...
		if (pegRevisionNumber < 0) {
			pegRevisionNumber = getRevisionNumber(revision, youngestRevNumber,
					repository, path);
		}
		if (revision == start && revision == SVNRevision.HEAD) {
			startRevisionNumber = pegRevisionNumber;
		} else {
			startRevisionNumber = getRevisionNumber(start, youngestRevNumber,
					repository, path);
		}
		if (!end.isValid()) {
			endRevisionNumber = startRevisionNumber;
		} else {
			endRevisionNumber = getRevisionNumber(end, youngestRevNumber,
					repository, path);
		}
		if (endRevisionNumber == pegRevisionNumber
				&& startRevisionNumber == pegRevisionNumber) {
		...
		try {
			locations = repository.getLocations(repoPath, (Map) null,
					pegRevisionNumber, revisionsRange);
		} catch (SVNException e) {
			if (e.getErrorMessage() != null
					&& e.getErrorMessage().getErrorCode() == SVNErrorCode.RA_NOT_IMPLEMENTED) {
				locations = getLocations10(repository, pegRevisionNumber,
						startRevisionNumber, endRevisionNumber);
			} else {
				throw e;
			}
		}
		SVNLocationEntry startPath = (SVNLocationEntry) locations.get(new Long(
				startRevisionNumber));
		SVNLocationEntry endPath = (SVNLocationEntry) locations.get(new Long(
				endRevisionNumber));
		if (startPath == null) {
		...
		if (endPath == null) {
			...
			SVNErrorMessage err = SVNErrorMessage.create(
					SVNErrorCode.CLIENT_UNRELATED_RESOURCES,
					"The location for ''{0}'' for revision {1} does not exist in the "
							+ "repository or refers to an unrelated object",
					new Object[] { source, new Long(endRevisionNumber) });
			SVNErrorManager.error(err, SVNLogType.WC);
		}
		SVNRepositoryLocation[] result = new SVNRepositoryLocation[2];
		SVNURL startURL = SVNURL.parseURIEncoded(SVNPathUtil.append(rootURL
				.toString(), SVNEncodingUtil.uriEncode(startPath.getPath())));
		result[0] = new SVNRepositoryLocation(startURL, startRevisionNumber);
		if (end.isValid()) {
			SVNURL endURL = SVNURL.parseURIEncoded(SVNPathUtil.append(rootURL
					.toString(), SVNEncodingUtil.uriEncode(endPath.getPath())));
			result[1] = new SVNRepositoryLocation(endURL, endRevisionNumber);
		}
		return result;
	}

	private Map getLocations10(SVNRepository repos, final long pegRevision,
		...
		if (kind == SVNNodeKind.NONE) {
			SVNErrorMessage err = SVNErrorMessage.create(
					...
					"path ''{0}'' doesn't exist at revision {1}", new Object[] {
		...
		LocationsLogEntryHandler handler = new LocationsLogEntryHandler(path,
				startRevision, endRevision, pegRevision, kind,
				getEventDispatcher());
		repos.log(new String[] { "" }, logStart, logEnd, true, false, handler);
		String pegPath = handler.myPegPath == null ? handler.myCurrentPath
				: handler.myPegPath;
		String startPath = handler.myStartPath == null ? handler.myCurrentPath
				: handler.myStartPath;
		String endPath = handler.myEndPath == null ? handler.myCurrentPath
				: handler.myEndPath;
		if (pegPath == null) {
			SVNErrorMessage err = SVNErrorMessage.create(
					SVNErrorCode.FS_NOT_FOUND,
					"path ''{0}'' in revision {1} is an unrelated object",
					new Object[] { path, new Long(logStart) });
			SVNErrorManager.error(err, SVNLogType.WC);
		}
		Map result = new SVNHashMap();
		result
				.put(new Long(startRevision), new SVNLocationEntry(-1,
						startPath));
		result.put(new Long(endRevision), new SVNLocationEntry(-1, endPath));
		return result;
	}

	private static String getPreviousLogPath(String path, SVNLogEntry logEntry,
			SVNNodeKind kind) throws SVNException {
		String prevPath = null;
		SVNLogEntryPath logPath = (SVNLogEntryPath) logEntry.getChangedPaths()
				.get(path);
		if (logPath != null) {
			if (logPath.getType() != SVNLogEntryPath.TYPE_ADDED
					&& logPath.getType() != SVNLogEntryPath.TYPE_REPLACED) {
			...
			for (Iterator paths = pathsList.iterator(); paths.hasNext();) {
				...
				if (path.startsWith(p + "/")) {
					...
					if (lPath.getCopyPath() != null) {
						prevPath = SVNPathUtil.append(lPath.getCopyPath(), path
								.substring(p.length()));
						break;
					}
				}
	...
	protected SVNURL getURL(File path) throws SVNException {
		return deriveLocation(path, null, null, SVNRevision.UNDEFINED, null,
				null);
	}

	protected SVNURL deriveLocation(File path, SVNURL url,
		...
		if (pegRevisionNumber != null && pegRevisionNumber.length > 0
			...
			try {
				...
				pegRevisionNumber[0] = getRevisionNumber(pegRevision, null,
						repos, path);
			} finally {
				if (closeRepository) {
					repos.closeSession();
	...
	protected SVNURL getEntryLocation(File path, SVNEntry entry, long[] revNum,
	...
	protected SVNURL ensureSessionURL(SVNRepository repository, SVNURL url)
			throws SVNException {
	...
	protected void setCommitItemProperty(SVNCommitItem item, String name,
			SVNPropertyValue value) {
		item.setProperty(name, value);
	}

	protected void setCommitItemFlags(SVNCommitItem item,
			boolean contentModified, boolean propertiesModified) {
