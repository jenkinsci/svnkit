...
public class SVNCommitClient16 extends SVNBasicDelegate {
	 ...
	 * <p/>{@link #getCommitHandler() Commit handler} will be asked for a commit log message.
	 * <p/>
	...
	public SVNCommitInfo doDelete(SVNURL[] urls, String commitMessage,
		...
		for (int i = 0; i < commitItems.length; i++) {
			...
			commitItems[i] = new SVNCommitItem(null, rootURL.appendPath(path,
					true), null, SVNNodeKind.NONE, SVNRevision.UNDEFINED,
					SVNRevision.UNDEFINED, false, true, false, false, false,
					false);
		}
	 ...
	 */
	public SVNCommitInfo doMkDir(SVNURL[] urls, String commitMessage,
			SVNProperties revisionProperties, boolean makeParents)
			throws SVNException {
		if (makeParents) {
		...
		for (int i = 0; i < commitItems.length; i++) {
			...
			commitItems[i] = new SVNCommitItem(null, rootURL.appendPath(path,
					true), null, SVNNodeKind.DIR, SVNRevision.UNDEFINED,
		...
		List decodedPaths = new ArrayList();
		for (Iterator commitPaths = sortedPaths.iterator(); commitPaths
				.hasNext();) {
			String path = (String) commitPaths.next();
			decodedPaths.add(SVNEncodingUtil.uriDecode(path));
		}
	 ...
	 */
	public SVNCommitInfo doImport(File path, SVNURL dstURL,
			String commitMessage, boolean recursive) throws SVNException {
		return doImport(path, dstURL, commitMessage, null, true, false,
	 ...
	 */
	public SVNCommitInfo doImport(File path, SVNURL dstURL,
			String commitMessage, boolean useGlobalIgnores, boolean recursive)
			throws SVNException {
		return doImport(path, dstURL, commitMessage, null, useGlobalIgnores,
				false, SVNDepth.fromRecurse(recursive));
	}
	 ...
	 * <p/>{@link #getCommitHandler() Commit handler} will be asked for a commit log message.
	 * <p/>
	 * If <code>depth</code> is {@link SVNDepth#EMPTY}, imports just <code>path</code> and nothing below it. If {@link SVNDepth#FILES}, imports <code>path</code> and any file children of <code>path</code>. If {@link SVNDepth#IMMEDIATES}, imports <code>path</code>, any file children, and any immediate 
	...
	public SVNCommitInfo doImport(File path, SVNURL dstURL,
		...
		if (newPaths.isEmpty()
			...
			SVNErrorMessage err = SVNErrorMessage.create(
					SVNErrorCode.ENTRY_EXISTS, "Path ''{0}'' already exists",
					dstURL);
			SVNErrorManager.error(err, SVNLogType.WC);
		}
		if (newPaths.contains(SVNFileUtil.getAdminDirectoryName())) {
			SVNErrorMessage err = SVNErrorMessage.create(
					...
					"''{0}'' is a reserved name and cannot be imported",
					SVNFileUtil.getAdminDirectoryName());
			SVNErrorManager.error(err, SVNLogType.WC);
		}
		...
		items[0] = new SVNCommitItem(path, dstURL, null,
				srcKind == SVNFileType.DIRECTORY ? SVNNodeKind.DIR
						: SVNNodeKind.FILE, SVNRevision.UNDEFINED,
		...
		SVNPropertiesManager.validateRevisionProperties(revisionProperties);
		ISVNEditor commitEditor = repos.getCommitEditor(commitMessage, null,
				false, revisionProperties, new SVNImportMediator());
		String filePath = "";
		...
		Collection ignores = useGlobalIgnores ? SVNStatusEditor
				.getGlobalIgnores(getOptions()) : null;
		checkCancelled();
		...
		try {
			...
			for (int i = newPaths.size() - 1; i >= 0; i--) {
				newDirPath = newDirPath == null ? (String) newPaths.get(i)
						: SVNPathUtil.append(newDirPath, (String) newPaths
								.get(i));
			...
			if (srcKind == SVNFileType.DIRECTORY) {
				changed |= importDir(deltaGenerator, path, newDirPath,
						useGlobalIgnores, ignoreUnknownNodeTypes, depth,
						commitEditor);
			} else if (srcKind == SVNFileType.FILE
					|| srcKind == SVNFileType.SYMLINK) {
				if (!useGlobalIgnores
						|| !SVNStatusEditor.isIgnored(ignores, path, "/"
								+ path.getName())) {
					changed |= importFile(deltaGenerator, path, srcKind,
							filePath, commitEditor);
				}
			} else if (srcKind == SVNFileType.NONE
					|| srcKind == SVNFileType.UNKNOWN) {
				SVNErrorMessage err = SVNErrorMessage.create(
	...
	public SVNCommitInfo doCommit(File[] paths, boolean keepLocks,
			...
			throws SVNException {
		return doCommit(paths, keepLocks, commitMessage, null, null, false,
				force, SVNDepth.getInfinityOrEmptyDepth(recursive));
	}
	 ...
	 * <p/>
	 * If the caller's {@link ISVNEventHandler event handler} is not <span class="javakeyword">null</span> it 
	 * will be called as the commit progresses with any of the following actions: {@link SVNEventAction#COMMIT_MODIFIED}, {@link SVNEventAction#COMMIT_ADDED}, {@link SVNEventAction#COMMIT_DELETED}, {@link SVNEventAction#COMMIT_REPLACED}. If the commit 
	 * succeeds, the handler will be called with {@link SVNEventAction#COMMIT_COMPLETED} event 
	 * action.
	 * <p/>
	...
	public SVNCommitInfo doCommit(File[] paths, boolean keepLocks,
		...
		try {
			...
			return doCommit(packet, keepLocks, keepChangelist, commitMessage,
					revisionProperties);
		} finally {
			if (packet != null) {
				packet.dispose();
	 ...
	 * Commit items are gathered into a single {@link SVNCommitPacket}by invoking {@link #doCollectCommitItems(File[],boolean,boolean,boolean) doCollectCommitItems()}. 
	 ...
	 * Commits files or directories into repository.
	 * <p/>
	 * This method is identical to {@link #doCommit(File[],boolean,String,SVNProperties,String[],boolean,boolean,SVNDepth)}except for it receives a commit packet instead of paths array. The aforementioned method collects commit 
	 ...
	 * either {@link #doCollectCommitItems(File[],boolean,boolean,SVNDepth,String[])} or {@link #doCollectCommitItems(File[],boolean,boolean,SVNDepth,boolean,String[])}. 
	 * <p/>
	...
	public SVNCommitInfo doCommit(SVNCommitPacket commitPacket,
			...
			SVNProperties revisionProperties) throws SVNException {
		SVNCommitInfo[] info = doCommit(new SVNCommitPacket[] { commitPacket },
				keepLocks, keepChangelist, commitMessage, revisionProperties);
		if (info != null && info.length > 0) {
			if (info[0].getErrorMessage() != null
					&& info[0].getErrorMessage().getErrorCode() != SVNErrorCode.REPOS_POST_COMMIT_HOOK_FAILED) {
	 ...
	 * that was changed and need to be committed to the repository) into a single {@link SVNCommitPacket}. 
	 * <p/>
	 ...
	 * items were found then {@link SVNCommitPacket#EMPTY} is returned
	 * @throws SVNException
	 * @deprecated              use {@link #doCollectCommitItems(File[],boolean,boolean,SVNDepth,String[])}instead
	 */
	 ...
	 * @throws SVNException
	 * @deprecated              use {@link #doCollectCommitItems(File[],boolean,boolean,SVNDepth,boolean,String[])}instead 
	 */
	public SVNCommitPacket[] doCollectCommitItems(File[] paths,
			boolean keepLocks, boolean force, boolean recursive,
			boolean combinePackets) throws SVNException {
		SVNDepth depth = recursive ? SVNDepth.INFINITY : SVNDepth.EMPTY;
		return doCollectCommitItems(paths, keepLocks, force, depth,
				combinePackets, null);
	}
	...
	public SVNCommitPacket[] doCollectCommitItems(File[] paths,
		...
		try {
			for (int i = 0; i < packetsArray.length; i++) {
				...
				File wcRoot = SVNWCUtil.getWorkingCopyRoot(packet
	...
	private boolean importDir(SVNDeltaGenerator deltaGenerator, File dir,
			String importPath, boolean useGlobalIgnores,
			boolean ignoreUnknownNodeTypes, SVNDepth depth, ISVNEditor editor)
			throws SVNException {
		checkCancelled();
		File[] children = SVNFileListUtil.listFiles(dir);
		boolean changed = false;
		ISVNFileFilter filter = getCommitHandler() instanceof ISVNFileFilter ? (ISVNFileFilter) getCommitHandler()
				: null;
		Collection ignores = useGlobalIgnores ? SVNStatusEditor
				.getGlobalIgnores(getOptions()) : null;
		for (int i = 0; children != null && i < children.length; i++) {
			...
			if (SVNFileUtil.getAdminDirectoryName().equals(file.getName())) {
				SVNEvent skippedEvent = SVNEventFactory.createSVNEvent(file,
						SVNNodeKind.NONE, null, SVNRepository.INVALID_REVISION,
						SVNEventAction.SKIP, SVNEventAction.COMMIT_ADDED, null,
						null);
			...
			if (useGlobalIgnores
					&& SVNStatusEditor.isIgnored(ignores, file, "/" + path)) {
				continue;
			}
			...
			if (fileType == SVNFileType.DIRECTORY
				...
				changed |= true;
				SVNEvent event = SVNEventFactory.createSVNEvent(file,
						SVNNodeKind.DIR, null, SVNRepository.INVALID_REVISION,
						SVNEventAction.COMMIT_ADDED, null, null, null);
				handleEvent(event, ISVNEventHandler.UNKNOWN);
				SVNDepth depthBelowHere = depth;
				if (depth == SVNDepth.IMMEDIATES) {
					depthBelowHere = SVNDepth.EMPTY;
				}
				importDir(deltaGenerator, file, path, useGlobalIgnores,
						ignoreUnknownNodeTypes, depthBelowHere, editor);
				editor.closeDir();
			} else if ((fileType == SVNFileType.FILE || fileType == SVNFileType.SYMLINK)
					&& depth.compareTo(SVNDepth.FILES) >= 0) {
				changed |= importFile(deltaGenerator, file, fileType, path,
						editor);
			} else if (fileType != SVNFileType.DIRECTORY
					&& fileType != SVNFileType.FILE) {
				if (ignoreUnknownNodeTypes) {
					SVNEvent skippedEvent = SVNEventFactory.createSVNEvent(
							file, SVNNodeKind.NONE, null,
							SVNRepository.INVALID_REVISION,
							SVNEventAction.SKIP, SVNEventAction.COMMIT_ADDED,
							null, null);
					handleEvent(skippedEvent, ISVNEventHandler.UNKNOWN);
				} else {
					SVNErrorMessage err = SVNErrorMessage.create(
	...
	private boolean importFile(SVNDeltaGenerator deltaGenerator, File file,
			SVNFileType fileType, String filePath, ISVNEditor editor)
			throws SVNException {
		...
		if (fileType != SVNFileType.SYMLINK) {
			autoProperties = SVNPropertiesManager.computeAutoProperties(
					getOptions(), file, autoProperties);
		} else {
			autoProperties.put(SVNProperty.SPECIAL, "*");
		}
		String mimeTypeProperty = (String) autoProperties
		...
		for (Iterator names = autoProperties.keySet().iterator(); names
			...
			if (SVNProperty.CHARSET.equals(name) && value != null) {
				...
				try {
					SVNTranslator.getCharset(value, mimeTypeProperty, filePath,
							getOptions());
				} catch (SVNException e) {
					continue;
				}
			}
			editor.changeFileProperty(filePath, name, SVNPropertyValue
					.create(value));
		}
		SVNEvent addedEvent = SVNEventFactory.createSVNEvent(file,
				SVNNodeKind.FILE, mimeTypeProperty,
				SVNRepository.INVALID_REVISION, SVNEventAction.COMMIT_ADDED,
				null, null, null);
		handleEvent(addedEvent, ISVNEventHandler.UNKNOWN);
		String charset = SVNTranslator.getCharset((String) autoProperties
				.get(SVNProperty.CHARSET), mimeTypeProperty, file.getPath(),
				getOptions());
		...
		if (charset != null || eolStyle != null || keywords != null || special) {
			...
			Map keywordsMap = keywords != null ? SVNTranslator.computeKeywords(
					keywords, null, null, null, null, getOptions()) : null;
			tmpFile = SVNFileUtil.createTempFile("import", ".tmp");
			SVNTranslator.translate(file, tmpFile, charset, eolBytes,
