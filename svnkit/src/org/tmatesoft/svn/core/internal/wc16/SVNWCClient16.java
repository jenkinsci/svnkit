...
public class SVNWCClient16 extends SVNBasicDelegate {
	 ...
	 * <ul>
	 * <li>{@link SVNRevision#BASE}<li>{@link SVNRevision#WORKING}<li>{@link SVNRevision#COMMITTED}</ul>
	 ...
	 * the file's {@link org.tmatesoft.svn.core.SVNProperty#KEYWORDS}property (if set) will be substituted, otherwise not
	...
	public void doGetFileContents(File path, SVNRevision pegRevision,
		...
		if ((!pegRevision.isValid() || pegRevision == SVNRevision.BASE || pegRevision == SVNRevision.WORKING)
			...
			SVNRepository repos = createRepository(null, path, null,
			...
			if (kind == SVNNodeKind.DIR) {
				SVNErrorMessage err = SVNErrorMessage.create(
						...
						"URL ''{0}'' refers to a directory", repos
								.getLocation());
				SVNErrorManager.error(err, SVNLogType.WC);
			}
			checkCancelled();
			if (!expandKeywords) {
				repos.getFile("", revNumber, null,
						new SVNCancellableOutputStream(dst, this));
			} else {
				...
				String charset = SVNTranslator.getCharset(properties
						.getStringValue(SVNProperty.CHARSET), mimeType, path
						.getPath(), getOptions());
				if (keywords != null || eol != null || charset != null) {
					String cmtRev = properties
							.getStringValue(SVNProperty.COMMITTED_REVISION);
					String cmtDate = properties
							.getStringValue(SVNProperty.COMMITTED_DATE);
					String author = properties
							.getStringValue(SVNProperty.LAST_AUTHOR);
					Map keywordsMap = SVNTranslator.computeKeywords(keywords,
							expandKeywords ? repos.getLocation().toString()
									: null, author, cmtDate, cmtRev,
							getOptions());
					OutputStream translatingStream = SVNTranslator
							.getTranslatingOutputStream(dst, charset,
									SVNTranslator.getEOL(eol, getOptions()),
									false, keywordsMap, expandKeywords);
					repos.getFile("", revNumber, null,
							new SVNCancellableOutputStream(translatingStream,
									getEventDispatcher()));
					try {
						...
						SVNErrorManager.error(SVNErrorMessage.create(
								SVNErrorCode.IO_ERROR, e.getMessage()),
								SVNLogType.WC);
					}
				} else {
					repos.getFile("", revNumber, null,
							new SVNCancellableOutputStream(dst,
									getEventDispatcher()));
				}
			}
			try {
	 ...
	 * ({@link java.io.File#createTempFile(java.lang.String,java.lang.String) createTempFile()}fails) necessary for file translating
	 * </ul>
	 * @see #doGetFileContents(File,SVNRevision,SVNRevision,boolean,OutputStream)
	 */
	public void doGetFileContents(SVNURL url, SVNRevision pegRevision,
			SVNRevision revision, boolean expandKeywords, OutputStream dst)
			throws SVNException {
		...
		if (nodeKind == SVNNodeKind.DIR) {
			SVNErrorMessage err = SVNErrorMessage.create(
					SVNErrorCode.CLIENT_IS_DIRECTORY,
					"URL ''{0}'' refers to a directory", url);
			SVNErrorManager.error(err, SVNLogType.WC);
		}
		checkCancelled();
		if (!expandKeywords) {
			repos.getFile("", revNumber, null, new SVNCancellableOutputStream(
					dst, this));
		} else {
			SVNProperties properties = new SVNProperties();
			repos.getFile("", revNumber, properties, null);
			...
			String charset = SVNTranslator.getCharset(properties
					.getStringValue(SVNProperty.CHARSET), mimeType, repos
					.getLocation().toDecodedString(), getOptions());
			...
			if (charset != null || keywords != null || eol != null) {
				...
				String author = properties
						.getStringValue(SVNProperty.LAST_AUTHOR);
				Map keywordsMap = SVNTranslator.computeKeywords(keywords,
						expandKeywords ? repos.getLocation().toString() : null,
						author, cmtDate, cmtRev, getOptions());
				OutputStream translatingStream = SVNTranslator
						.getTranslatingOutputStream(dst, charset, SVNTranslator
								.getEOL(eol, getOptions()), false, keywordsMap,
								expandKeywords);
				repos.getFile("", revNumber, null,
						new SVNCancellableOutputStream(translatingStream,
								getEventDispatcher()));
	 ...
	 * <p/>
	 * The {@link ISVNAuthenticationManager authentication manager} and {@link ISVNCommitHandler commit handler}, either provided by a caller or default ones,
	 ...
	 * <span class="javastring">"svn:mime-type"</span>, but <code>propVal</code> is not a valid mime-type).
	 * <p/>
	 * If non-<span class="javakeyword">null</span>, <code>revisionProperties</code> is an {@link SVNProperties} object holding additional, custom revision properties 
	 ...
	 * if <code>propName</code> is either equal to {@link SVNProperty#EOL_STYLE} or {@link SVNProperty#KEYWORDS} or {@link SVNProperty#CHARSET}</ul>
	 * @see #doSetProperty(File,String,SVNPropertyValue,boolean,SVNDepth,ISVNPropertyHandler,Collection)
	 * @since                      1.2, SVN 1.5
	 */
	public SVNCommitInfo doSetProperty(SVNURL url, String propName,
			SVNPropertyValue propValue, SVNRevision baseRevision,
			String commitMessage, SVNProperties revisionProperties,
			boolean skipChecks, ISVNPropertyHandler handler)
			throws SVNException {
		if (propValue != null
				&& !SVNPropertiesManager.isValidPropertyName(propName)) {
			SVNErrorMessage err = SVNErrorMessage.create(
		...
		try {
			...
			SVNErrorMessage err = SVNErrorMessage
					.create(
							...
							"Setting property on non-local target ''{0}'' needs a base revision",
							url);
			SVNErrorManager.error(err, SVNLogType.WC);
		}
		if (SVNProperty.EOL_STYLE.equals(propName)
				|| SVNProperty.KEYWORDS.equals(propName)
				|| SVNProperty.CHARSET.equals(propName)) {
			SVNErrorMessage err = SVNErrorMessage
					.create(
		...
		if (propValue != null && SVNProperty.isSVNProperty(propName)) {
			...
			propValue = SVNPropertiesManager.validatePropertyValue(url
					.toString(), kind, propName, propValue, skipChecks,
					getOptions(), new ISVNFileContentFetcher() {
						Boolean isBinary = null;

						public void fetchFileContent(OutputStream os)
								throws SVNException {
						...
						private void setBinary(SVNProperties props) {
							String mimeType = props
									.getStringValue(SVNProperty.MIME_TYPE);
							isBinary = Boolean.valueOf(SVNProperty
									.isBinaryMimeType(mimeType));
						}
					});
		}
		Collection commitItems = new ArrayList(2);
		SVNCommitItem commitItem = new SVNCommitItem(null, url, null, kind,
				SVNRevision.create(revNumber), SVNRevision.UNDEFINED, false,
	 ...
	 * <p/>
	 * This method simply obtains a url given a working path and calls {@link #doSetRevisionProperty(SVNURL,SVNRevision,String,SVNPropertyValue,boolean,ISVNPropertyHandler)}passing this url and the rest parameters.
	 * @param path            working copy path
	 ...
	 * <li>exceptions thrown by {@link #doSetRevisionProperty(SVNURL,SVNRevision,String,SVNPropertyValue,boolean,ISVNPropertyHandler)}</ul>
	 */
	public void doSetRevisionProperty(File path, SVNRevision revision,
			String propName, SVNPropertyValue propValue, boolean force,
		...
		doSetRevisionProperty(url, revision, propName, propValue, force,
				handler);
	}
	 ...
	 */
	public void doSetRevisionProperty(SVNURL url, SVNRevision revision,
			String propName, SVNPropertyValue propValue, boolean force,
			ISVNPropertyHandler handler) throws SVNException {
		if (propValue != null
				&& !SVNPropertiesManager.isValidPropertyName(propName)) {
		...
		if (!force && SVNRevisionProperty.AUTHOR.equals(propName)
				&& propValue != null && propValue.isString()
				&& propValue.getString().indexOf('\n') >= 0) {
			SVNErrorMessage err = SVNErrorMessage.create(
		...
		SVNRepository repos = createRepository(url, null, null,
				SVNRevision.UNDEFINED, revision, null);
		long revNumber = getRevisionNumber(revision, repos, null);
		repos.setRevisionPropertyValue(revNumber, propName, propValue);
		if (handler != null) {
			handler.handleProperty(revNumber, new SVNPropertyData(propName,
					propValue, getOptions()));
		}
	}
	 ...
	 * with the {@link org.tmatesoft.svn.core.SVNProperty#SVN_WC_PREFIXsvn:wc:} prefix
	 * <li><code>path</code> is not under version control
	 * </ul>
	...
	public SVNPropertyData doGetProperty(final File path, String propName,
		...
		doGetProperty(path, propName, pegRevision, revision, SVNDepth.EMPTY,
	...
	public SVNPropertyData doGetProperty(final SVNURL url, String propName,
			SVNRevision pegRevision, SVNRevision revision) throws SVNException {
		final SVNPropertyData[] data = new SVNPropertyData[1];
		doGetProperty(url, propName, pegRevision, revision, SVNDepth.EMPTY,
				new ISVNPropertyHandler() {
					public void handleProperty(File file,
							SVNPropertyData property) {
					}
					...
					public void handleProperty(SVNURL location,
							SVNPropertyData property) throws SVNException {
						if (data[0] == null
								&& url.toString().equals(location.toString())) {
							data[0] = property;
						}
					}
				});
	 ...
	 * If <vode>revision</code> is one of:
	 * <ul>
	 * <li>{@link SVNRevision#BASE BASE}<li>{@link SVNRevision#WORKING WORKING}<li>{@link SVNRevision#COMMITTED COMMITTED}</ul>
	...
	public void doGetProperty(File path, String propName,
			...
			ISVNPropertyHandler handler) throws SVNException {
		doGetProperty(path, propName, pegRevision, revision, SVNDepth
				.getInfinityOrEmptyDepth(recursive), handler, null);
	}
	...
	public void doGetProperty(SVNURL url, String propName,
			SVNRevision pegRevision, SVNRevision revision, boolean recursive,
			ISVNPropertyHandler handler) throws SVNException {
		doGetProperty(url, propName, pegRevision, revision, SVNDepth
				.getInfinityOrEmptyDepth(recursive), handler);
	}
	 ...
	 * <li><code>propName</code> starts with the{@link org.tmatesoft.svn.core.SVNProperty#SVN_WC_PREFIXsvn:wc:} prefix
	 * </ul>
	 ...
	 * <p/>
	 * If <code>force</code> is set, path is a directory, <code>depth</code> is {@link SVNDepth#INFINITY}, then schedules for addition unversioned files and directories
	 * scattered deep within a versioned tree.
	 * <p/>
	 * If <code>includeIgnored</code> is <span class="javakeyword">false</span>, doesn't add files or 
	 * directories that match ignore patterns.
	 ...
	 * @since 1.3
	 */
	public void doAdd(File[] paths, boolean force, boolean mkdir,
			boolean climbUnversionedParents, SVNDepth depth,
		...
		try {
			if (paths != null) {
				for (int i = 0; i < paths.length; i++) {
					doAdd(paths[i], force, mkdir, climbUnversionedParents,
							depth, depthIsSticky, includeIgnored, makeParents);
				}
			}
		} finally {
	...
	public void doAdd(File path, boolean force, boolean mkdir,
		...
		try {
			...
			if (fileType == SVNFileType.DIRECTORY) {
				dir = wcAccess.open(SVNWCUtil.isVersionedDirectory(path
	...
	public void doRevert(File path, boolean recursive) throws SVNException {
		doRevert(new File[] { path }, SVNDepth
				.getInfinityOrEmptyDepth(recursive), null);
	}
	...
	public void doResolve(File path, boolean recursive) throws SVNException {
		doResolve(path, SVNDepth.fromRecurse(recursive),
	...
	public void doResolve(File path, SVNDepth depth,
			...
			SVNConflictChoice conflictChoice) throws SVNException {
		doResolve(path, depth, resolveContents, resolveProperties, true,
				conflictChoice);
	}
	...
	public void doLock(SVNURL[] urls, boolean stealLock, String lockMessage)
		...
		repository.lock(pathsToRevisions, lockMessage, stealLock,
				new ISVNLockHandler() {
					public void handleLock(String path, SVNLock lock,
							SVNErrorMessage error) throws SVNException {
						if (error != null) {
							handleEvent(SVNEventFactory.createLockEvent(
									new File(path), SVNEventAction.LOCK_FAILED,
									lock, error), ISVNEventHandler.UNKNOWN);
						} else {
							handleEvent(SVNEventFactory.createLockEvent(
									new File(path), SVNEventAction.LOCKED,
									lock, null), ISVNEventHandler.UNKNOWN);
						}
					}
	...
	public void doUnlock(SVNURL[] urls, boolean breakLock) throws SVNException {
		...
		repository.unlock(pathsToTokens, breakLock, new ISVNLockHandler() {
			...
			public void handleUnlock(String path, SVNLock lock,
					SVNErrorMessage error) throws SVNException {
				if (error != null) {
					handleEvent(SVNEventFactory.createLockEvent(new File(path),
							SVNEventAction.UNLOCK_FAILED, null, error),
							ISVNEventHandler.UNKNOWN);
				} else {
					handleEvent(SVNEventFactory.createLockEvent(new File(path),
	...
	public void doInfo(File path, SVNRevision revision, boolean recursive,
		...
		doInfo(path, SVNRevision.UNDEFINED, revision, SVNDepth
				.getInfinityOrEmptyDepth(recursive), null, handler);
	}
	...
	public void doInfo(File path, SVNRevision pegRevision,
			...
			throws SVNException {
		doInfo(path, pegRevision, revision, SVNDepth
				.getInfinityOrEmptyDepth(recursive), null, handler);
	}
	...
	public void doInfo(SVNURL url, SVNRevision pegRevision,
			SVNRevision revision, boolean recursive, ISVNInfoHandler handler)
			throws SVNException {
		doInfo(url, pegRevision, revision, SVNDepth
				.getInfinityOrEmptyDepth(recursive), handler);
	}
	...
	public SVNInfo doInfo(File path, SVNRevision revision) throws SVNException {
		...
		doInfo(path, SVNRevision.UNDEFINED, revision, SVNDepth.EMPTY, null,
				new ISVNInfoHandler() {
	...
	public SVNInfo doInfo(SVNURL url, SVNRevision pegRevision,
		...
		final SVNInfo[] result = new SVNInfo[1];
		doInfo(url, pegRevision, revision, SVNDepth.EMPTY,
				new ISVNInfoHandler() {
					public void handleInfo(SVNInfo info) {
						if (result[0] == null) {
							result[0] = info;
	 ...
	 */
	public void doSetProperty(File path, String propName,
			SVNPropertyValue propValue, boolean force, boolean recursive,
			ISVNPropertyHandler handler) throws SVNException {
		doSetProperty(path, propName, propValue, force, SVNDepth
				.getInfinityOrEmptyDepth(recursive), handler, null);
	}

	private SVNAdminArea addParentDirectories(SVNWCAccess wcAccess, File path)
			throws SVNException {
		try {
	...
	private void doGetRevisionProperty(SVNRepository repos, String propName,
			long revNumber, ISVNPropertyHandler handler) throws SVNException {
		if (propName != null) {
			SVNPropertyValue value = repos.getRevisionPropertyValue(revNumber,
					propName);
			if (value != null) {
				handler.handleProperty(revNumber, new SVNPropertyData(propName,
						value, getOptions()));
			...
			for (Iterator names = props.nameSet().iterator(); names.hasNext();) {
				...
				handler.handleProperty(revNumber, new SVNPropertyData(name,
						value, getOptions()));
			}
		}
	}

	private void addDirectory(File wcRoot, File path, SVNAdminArea parentDir,
	...
	private void addFile(File path, SVNFileType type, SVNAdminArea dir)
			throws SVNException {
	...
	private SVNURL collectLockInfo(SVNWCAccess wcAccess, File[] files,
		...
		for (int i = 0; i < paths.length; i++) {
			...
			if (lock) {
				...
				lockInfo.put(entry.getSVNURL(), new LockInfo(file, stealLock ? null : entry
						.getLockToken()));
			}
		}
		checkCancelled();
	...
	private void crawlEntries(File path, SVNDepth depth,
	...
	private void reportEntry(File path, SVNEntry entry, ISVNInfoHandler handler)
			throws SVNException {
	...
	private boolean doRevert(File path, SVNAdminArea parent, SVNDepth depth,
	...
	private boolean revert(SVNAdminArea dir, String name, SVNEntry entry,
	...
	private void setWCFormat(SVNAdminAreaInfo info, SVNAdminArea area,
			int format) throws SVNException {
	...
	private void pushDirInfo(SVNRepository repos, SVNRevision rev, String path,
	...
	private void doGetLocalProperty(SVNEntry entry, SVNAdminArea area,
	...
	private void setLocalProperties(File path, SVNEntry entry,
	...
	private Map fetchLockTokens(SVNRepository repository, Map pathsTokensMap)
			throws SVNException {
		Map tokens = new SVNHashMap();
		for (Iterator paths = pathsTokensMap.keySet().iterator(); paths
				.hasNext();) {
			String path = (String) paths.next();
	...
	private void doGetLocalFileContents(File path, OutputStream dst,
		...
		try {
			...
			String mimeType = properties
					.getStringPropertyValue(SVNProperty.MIME_TYPE);
