...
public class SVNCopyClient16 extends SVNCopyDriver {
	 ...
	 */
	public SVNCopyClient16(ISVNAuthenticationManager authManager,
			ISVNOptions options) {
	 ...
	 * @param options          a run-time configuration options driver
	 */
	public SVNCopyClient16(ISVNRepositoryPool repositoryPool,
			ISVNOptions options) {
	 ...
	 * <p>
	 * When no parameters are set {@link DefaultSVNCommitParameters default}ones are used.
	 ...
	 * <p/>
	 * For more information what externals handlers are for, please, refer to {@link ISVNExternalsHandler} and{@link #doCopy(SVNCopySource[],SVNURL,boolean,boolean,boolean,String,SVNProperties)}.
	 ...
	 * <p/>
	 * If <code>sources</code> has multiple items, and <code>failWhenDstExists</code> is 
	 * <span class="javakeyword">true</span>, throws an {@link SVNException} with the {@link SVNErrorCode#CLIENT_MULTIPLE_SOURCES_DISALLOWED}.
	 * <p/>
	 ...
	 * @since                        1.2, SVN 1.5
	 */
	public void doCopy(SVNCopySource[] sources, File dst, boolean isMove,
		...
		if (sources.length > 1 && failWhenDstExists) {
			SVNErrorMessage err = SVNErrorMessage
					.create(SVNErrorCode.CLIENT_MULTIPLE_SOURCES_DISALLOWED);
		...
		try {
			setupCopy(sources, new SVNPath(dst.getAbsolutePath()), isMove,
					makeParents, null, null, getCommitHandler(),
					getCommitParameters(), getExternalsHandler());
		} catch (SVNException e) {
			SVNErrorCode err = e.getErrorMessage().getErrorCode();
			if (!failWhenDstExists
					&& sources.length == 1
					&& (err == SVNErrorCode.ENTRY_EXISTS || err == SVNErrorCode.FS_ALREADY_EXISTS)) {
				...
				try {
					setupCopy(sources, new SVNPath(new File(dst, baseName)
							.getAbsolutePath()), isMove, makeParents, null,
							null, getCommitHandler(), getCommitParameters(),
							getExternalsHandler());
	 ...
	 * If <code>sources</code> has multiple items, and <code>failWhenDstExists</code> is 
	 * <span class="javakeyword">true</span>, throws an {@link SVNException} with the {@link SVNErrorCode#CLIENT_MULTIPLE_SOURCES_DISALLOWED}.
	 * <p/>{@link ISVNAuthenticationManager Authentication manager} (whether provided directly through the 
	 ...
	 * revisions during a wc-to-url copy. To enable externals revision fixing a user should implement {@link ISVNExternalsHandler}. The user's implementation {@link ISVNExternalsHandler#handleExternal(File,SVNURL,SVNRevision,SVNRevision,String,SVNRevision)} 
	 ...
	 * @since                        1.2, SVN 1.5
	 */
	public SVNCommitInfo doCopy(SVNCopySource[] sources, SVNURL dst,
		...
		try {
			return setupCopy(sources, new SVNPath(dst.toString()), isMove,
					makeParents, commitMessage, revisionProperties,
					getCommitHandler(), getCommitParameters(),
					getExternalsHandler());
		} catch (SVNException e) {
			SVNErrorCode err = e.getErrorMessage().getErrorCode();
			if (!failWhenDstExists
				...
				try {
					return setupCopy(sources, new SVNPath(dst.appendPath(
							baseName, true).toString()), isMove, makeParents,
							commitMessage, revisionProperties,
							getCommitHandler(), getCommitParameters(),
							getExternalsHandler());
				} catch (SVNException second) {
					throw second;
