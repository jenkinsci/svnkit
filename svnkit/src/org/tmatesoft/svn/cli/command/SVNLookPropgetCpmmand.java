 ...
 */
public class SVNLookPropgetCpmmand extends SVNCommand {
    ...
    public void run(PrintStream out, PrintStream err) throws SVNException {
        ...
        if (getCommandLine().hasArgument(SVNArgument.TRANSACTION)) {
            ...
            if (value == null) {
                if (path == null) {
                    SVNCommand.println(out, "Property '" + propertyName + "' not found on transaction '" + transactionName + "'");
                    System.exit(1);
                } else {
                    SVNCommand.println(out, "Property '" + propertyName + "' not found on path '" + path + "' in transaction '" + transactionName + "'");
                    System.exit(1);
                }
            }
        ...
        if (value == null) {
            ...
            if (path == null) {
                SVNCommand.println(out, "Property '" + propertyName + "' not found on revision " + revNum);
                System.exit(1);
            } else {
                SVNCommand.println(out, "Property '" + propertyName + "' not found on path '" + path + "' in revision " + revNum);
                System.exit(1);
            }
        }
        SVNCommand.print(out, value);
