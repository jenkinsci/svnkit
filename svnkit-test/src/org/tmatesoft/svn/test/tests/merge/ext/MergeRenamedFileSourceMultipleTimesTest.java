 ...
 * @version 1.2.0
 */
public class MergeRenamedFileSourceMultipleTimesTest extends AbstractExtMergeTest {
    ...
    public void run() throws SVNException {
        ...
        getEnvironment().commit(getBranchWC(), "A/file renamed to A/file2", SVNDepth.INFINITY);
        getEnvironment().modifyAndCommit(getBranchFile("A/file2"));

        getEnvironment().copy(getBranchFile("A/file2"), SVNRevision.WORKING, getBranchFile("A/file3"), true, false, true);
        getEnvironment().commit(getBranchWC(), "A/file2 renamed to A/file3", SVNDepth.INFINITY);
        getEnvironment().modifyAndCommit(getBranchFile("A/file3"));
...
//
//        getEnvironment().revert(getTrunkWC());
//
//        mergeLastRevisions(getBranch(), getTrunkWC(), 6, SVNDepth.INFINITY, false, false);
