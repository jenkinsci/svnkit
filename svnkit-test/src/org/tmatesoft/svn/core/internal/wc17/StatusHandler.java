...
public class StatusHandler implements ISVNStatusHandler, ISVNEventHandler {
    ...
    public void handleStatus(SVNStatus status) {
        ...
        if (contentsStatus == SVNStatusType.STATUS_MODIFIED) {
             ...
             * from the filesystem). In the case the item is a directory, it can
             * be incomplete if the user happened to interrupt a checkout or
             * update.
             */
            pathChangeType = "!";
        } else if (contentsStatus == SVNStatusType.STATUS_OBSTRUCTED) {
            /*
             * The file, directory or symbolic link item is in the repository as
             * one kind of object, but what's actually in the user's working
             * copy is some other kind. For example, Subversion might have a
             * file in the repository, but the user removed the file and created
             * a directory in its place, without using the 'svn delete' or 'svn
             * add' command (or SVNKit analogues for them).
             */
            pathChangeType = "~";
         ...
         * following code finds out whether the current item had been changed in
         * the repository
         */
        ...
        /*
         * If doStatus() was run with remote=true and the item is a file, checks
         ...
         */
        System.out.println(pathChangeType + propertiesChangeType + (isLocked ? "L" : " ") + (isAddedWithHistory ? "+" : " ") + (isSwitched ? "S" : " ") + lockLabel + "  " + remoteChangeType + "  "
                + (workingRevision > 0 ? workingRevision : " ") + offsets[0] + (lastChangedRevision >= 0 ? String.valueOf(lastChangedRevision) : "?") + offsets[1]
                + (status.getAuthor() != null ? status.getAuthor() : "?") + offsets[2] + status.getFile().getPath());
    }

    /*
     * This is an implementation for ISVNEventHandler.handleEvent(SVNEvent
     * event, double progress)
     */
