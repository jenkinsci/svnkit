...
public class SVNInfo {
    ...
    static SVNInfo createInfo(File file, SVNEntry entry) {
        ...
        if (entry.getLockToken() != null) {
            lock = new SVNLock(null, entry.getLockToken(), entry.getLockOwner(), entry.getLockComment(),
                    TimeUtil.parseDate(entry.getLockCreationDate()), null);
        }
        return new SVNInfo(file, entry.getURL(), entry.getRevision(), entry.getKind(), 
                entry.getUUID(), entry.getCommittedRevision(), entry.getCommittedDate(),
                entry.getAuthor(), entry.getSchedule(), entry.getCopyFromURL(), 
                entry.getCopyFromRevision(), entry.getTextTime(), entry.getPropTime(),
                entry.getChecksum(), entry.getConflictOld(), entry.getConflictNew(), 
                entry.getConflictWorking(), entry.getPropRejectFile(), lock);
    }

    static SVNInfo createInfo(String path, String reposRootURL, String uuid, String url, SVNRevision revision, SVNDirEntry dirEntry, SVNLock lock) {
        if (dirEntry == null) {
            return null;
        }
    ...
    protected SVNInfo(File file, String url, long revision, SVNNodeKind kind, String uuid, 
            long committedRevision, String committedDate, String author, 
            String schedule, 
            String copyFromURL, long copyFromRevision, 
            String textTime, String propTime, 
            String checksum, 
            String conflictOld, String conflictNew, String conflictWorking, String propRejectFile, 
            SVNLock lock) {
        myFile = file;
        myURL = url;
        ...
        if (file != null) {
            ...
            if (conflictWorking != null) {
                myConflictWrkFile = new File(file.getParentFile(), conflictWorking);
            }
            if (propRejectFile != null) {
                myPropConflictFile = new File(file.getParentFile(), propRejectFile);
            }
        }
    ...
    protected SVNInfo(String path, String url, SVNRevision revision, SVNNodeKind kind, String uuid, String reposRootURL, 
