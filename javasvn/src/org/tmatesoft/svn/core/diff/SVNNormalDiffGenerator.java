...
public class SVNNormalDiffGenerator extends SVNSequenceDiffGenerator implements ISVNDiffGeneratorFactory {
    ...
    protected void processBlock(QSequenceDifferenceBlock[] segment, SVNSequenceLine[] sourceLines, SVNSequenceLine[] targetLines, String encoding, Writer output)
            throws IOException {
    ...
    protected String displayWhiteSpace(String s) {
        if (Boolean.TRUE.toString().equals(getProperties().get(ISVNDiffGeneratorFactory.WHITESPACE_PROPERTY))) {
            s = s.replaceAll("\t", "<tb>");
            s = s.replaceAll(" ", ".");
        }
        return s;
     ...
     * 
     * @param deleteAt
     *            the line where the lines would have appeared in the source (0
     *            based)
     * @param deleteStart
     *            first line deleted from target (0 based).
     * @param deleteEnd
     *            last line deleted from target (0 based).
     * @param deleteLines
     *            all the lines from the target file. Could be accessed with
     *            <CODE>deleteStart</CODE> and <CODE>deleteEnd</CODE> to
     *            identify the deleted lines.
     */
     ...
     * 
     * @param addAt
     *            the line where the new lines would be added to target (0
     *            based)
     * @param addStart
     *            the first line added from source (0 based)
     * @param addEnd
     *            the last line added from source (0 based)
     * @param addLines
     *            all the lines from the source file. Could be accessed with
     *            <CODE>addStart</CODE> and <CODE>addEnd</CODE> to identify
     *            the added lines.
     */
    ...
    /**
     * Handles a change of a range of lines in target to a range of lines in
     * source.
     * 
     * @param replaceStart
     *            the first line in target that will be replaced (0 based)
     * @param replaceEnd
     *            the last line in target that will be replaced (0 based)
     * @param replaceLines
     *            all the lines in target
     * @param replaceWithStart
     *            the first line in source to that will replace the lines in
     *            target (0 based)
     * @param replaceWithEnd
     *            the last line in source to that will replace the lines in
     *            target (0 based)
     * @param replaceWithLines
     *            all the lines in source
     */
    protected void change(int replaceStart, int replaceEnd, SVNSequenceLine[] replaceLines, int replaceWithStart, int replaceWithEnd,
            SVNSequenceLine[] replaceWithLines, String encoding, Writer output) throws IOException {
        /*
