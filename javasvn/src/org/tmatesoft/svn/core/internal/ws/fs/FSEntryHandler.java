...
public class FSEntryHandler {
    ...
    public static void save(Writer os, Map entry, Collection childEntries,
            Collection deletedEntries) throws IOException {
    ...
    private static void saveEntry(Writer os, Map parent, Map entry)
            throws IOException {
        os.write("<entry\n");
        boolean first = true;
        for (Iterator entries = entry.entrySet().iterator(); entries.hasNext();) {
            ...
            if (parent != null) {
                if (SVNProperty.REVISION.equals(e.getKey())
                        && e.getValue()
                                .equals(parent.get(SVNProperty.REVISION))) {
                    continue;
                }
                if (SVNProperty.COPYFROM_REVISION.equals(e.getKey())
                        && e.getValue().equals(
                                parent.get(SVNProperty.COPYFROM_REVISION))) {
                    continue;
                }
                if ((SVNProperty.REVISION.equals(e.getKey()) || SVNProperty.URL
                        .equals(e.getKey()))
                        && SVNProperty.KIND_DIR.equals(entry
                                .get(SVNProperty.KIND))) {
                    // do not save revision and url for child directories
                    continue;
                }
                if (SVNProperty.UUID.equals(e.getKey())
                        && e.getValue().equals(parent.get(SVNProperty.UUID))) {
                    continue;
                }
                if (SVNProperty.URL.equals(e.getKey())
                        || SVNProperty.COPYFROM_URL.equals(e.getKey())) {
                    String name = (String) entry.get(SVNProperty.NAME);
                    name = xmlDecode(name);
                    String url = (String) parent.get(e.getKey());
                    String expected = PathUtil.append(url, PathUtil
                            .encode(name));
            ...
            first = false;
            os.write("   ");
            os.write(e.getKey().toString().substring(
                    FSEntry.ENTRY_PREFIX.length()));
            os.write("=\"");
