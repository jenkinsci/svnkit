...
public class FSMerger {
    ...
    public int pretendMergeFiles(File base, File local, File latest)
            throws SVNException {
    ...
    public int mergeFiles(File base, File local, File latest, File result,
            String start, String end) throws SVNException {
        if (base == null || local == null || latest == null) {
            throw new SVNException("At least one file is missing, can't merge");
        }
    ...
    private int doMergeFiles(File base, File local, File latest, File result,
            String start, String end) throws SVNException {
        final String conflictStart = "<<<<<<< " + start;
        final String conflictEnd = ">>>>>>> " + end;
        final String conflictSeparator = "=======";

        final FSMergerBySequence merger = new FSMergerBySequence(conflictStart
                .getBytes(), conflictSeparator.getBytes(), conflictEnd
                .getBytes(), EOL.getBytes());
        try {
            final OutputStream resultStream = result != null ? (OutputStream) new FileOutputStream(
                    result)
            ...
            try {
                return merger.merge(new QSequenceLineRAFileData(raBase),
                        new QSequenceLineRAFileData(raLocal),
                        new QSequenceLineRAFileData(raLatest), resultStream);
            } finally {
                resultStream.close();
                raBase.close();
                raLocal.close();
