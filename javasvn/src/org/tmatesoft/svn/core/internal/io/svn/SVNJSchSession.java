...
public class SVNJSchSession {
	...
	static Session getSession(SVNRepositoryLocation location, SVNAuthentication credentials) throws SVNAuthenticationException {
		...
		String key = credentials.getUserName() + ":" + location.getHost() + ":" + location.getPort();
		...
		try {
			if (session == null) {
				...
				session = jsch.getSession(credentials.getUserName(), location.getHost(), location.getPort());
