...
public class FSNodeHistory {
    ...
    public static boolean checkAncestryOfPegPath(File reposRootDir, String fsPath, long pegRev, long futureRev, FSRevisionNodePool revNodesPool) throws SVNException {
        ...
        /*
         * We must have had at least one iteration above where we reassigned
         * fsPath. Else, the path wouldn't have existed at futureRev.
         */
        if (fsPath == null) {
    ...
    public static FSNodeHistory getNodeHistory(File reposRootDir, FSRoot root, String path, FSRevisionNodePool pool) throws SVNException {
        ...
        // SVNNodeKind kind = FSReader.checkNodeKind(path,
        // root.getRootRevisionNode(), reposRootDir);
        SVNNodeKind kind = null;
        try {
            node = pool.openPath(root, path, false, null, reposRootDir, false).getRevNode(); // FSReader.getRevisionNode(reposRootDir,
                                                                                                // path,
                                                                                                // root,
                                                                                                // 0);
    ...
    private FSNodeHistory historyPrev(File reposRootDir, boolean crossCopies, FSRevisionNodePool revNodesPool) throws SVNException {
        ...
        /*
         * The Subversion filesystem is written in such a way that a given line
         * of history may have at most one interesting history point per
         * filesystem revision. Either that node was edited (and possibly
         * copied), or it was copied but not edited. And a copy source cannot be
         * from the same revision as its destination. So, if our history
         * revision matches its node's commit revision, we know that ...
         */
        FSNodeHistory prevHist = null;
        if (revision == commitEntry.getRevision()) {
            /*
             * ... we either have not yet reported on this revision (and need
             * now to do so) ...
             */
            if (!reported) {
            ...
            /*
             * ... or we *have* reported on this revision, and must now progress
             * toward this node's predecessor (unless there is no predecessor,
             * in which case we're all done!).
             */
        ...
        if (copyrootEntry.getRevision() > commitEntry.getRevision()) {
             ...
             * our new current path will be the copy source. If our current path
             * was instead the *child* of the destination of the copy, then
             ...
             * criteria ... ### for now just fallback to the old copy hunt
             * algorithm.
             */
            ...
            if (reminder != null) {
                /*
                 * If we get here, then our current path is the destination of,
                 * or the child of the destination of, a copy. Fill in the
                 * return values and get outta here.
                 */
        ...
        if (srcEntry.getPath() != null && FSRepository.isValidRevision(srcEntry.getRevision())) {
            /*
             * It's possible for us to find a copy location that is the same as
             * the history point we've just reported. If that happens, we simply
             * need to take another trip through this history search
             */
            boolean retry = false;
    ...
    public FSNodeHistory fsHistoryPrev(File reposRootDir, boolean crossCopies, FSRevisionNodePool revNodesPool) throws SVNException {
        /*
         * Special case: the root directory changes in every single revision, no
         * exceptions. And, the root can't be the target (or child of a target --
         * duh) of a copy. So, if that's our path, then we need only decrement
         * our revision by 1, and there you go.
         */
