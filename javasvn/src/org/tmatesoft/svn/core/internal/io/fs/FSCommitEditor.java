...
public class FSCommitEditor implements ISVNEditor {
    ...
    public void openRoot(long revision) throws SVNException {
        /*
         * Ignore revision. We always build our transaction against HEAD.
         * However, we will keep it in dir baton for out of dateness checks.
         */
        ...
        if (isTxnOwner) {
            ...
            /*
             * Even if we aren't the owner of the transaction, we might have
             * been instructed to set some properties.
             */
            /* User (author). */
    ...
    private FSTransactionInfo beginTransactionForCommit(long baseRevision) throws SVNException {
        ...
        /*
         * We pass the author and log message to the filesystem by adding them
    ...
    public void openDir(String path, long revision) throws SVNException {
        ...
        /*
         * Check path in our transaction. If it does not exist, return a 'Path
         * not present' error.
         */
    ...
    public void deleteEntry(String path, long revision) throws SVNException {
        ...
        SVNNodeKind kind = myRepository.checkNodeKind(fullPath, myTxnRoot);
        /*
         * If path doesn't exist in the txn, that's fine (merge allows this).
         */
        if (kind == SVNNodeKind.NONE) {
            return;
        }
        /*
         * Now, make sure we're deleting the node that we think we're deleting,
    ...
    private void deleteNode(FSOldRoot root, String path) throws SVNException {
        ...
        /*
         * Check to see if path (or any child thereof) is locked; if so, check
         * that we can use the existing lock(s).
         */
    ...
    public void addDir(String path, String copyFromPath, long copyFromRevision) throws SVNException {
        ...
        if (copyFromPath != null && FSRepository.isInvalidRevision(copyFromRevision)) {
            ...
            /*
             * Check path in our transaction. Make sure it does not exist unless
             * its parent directory was copied (in which case, the thing might
             * have been copied in as well), else return an out-of-dateness
             * error.
             */
            SVNNodeKind kind = myRepository.checkNodeKind(fullPath, myTxnRoot);
            ...
            /*
             * No ancestry given, just make a new directory. We don't bother
             * with an out-of-dateness check here because makeDir() will error
             * out if path already exists.
             */
    ...
    /*
     * Create a new directory named "path" in "root". The new directory has no
    ...
    private void makeDir(FSOldRoot root, String path) throws SVNException {
        ...
        /*
         * If there's already a sub-directory by that name, complain. This also
         * catches the case of trying to make a subdirectory named `/'.
         */
        if (parentPath.getRevNode() != null) {
            SVNErrorManager.error(FSErrors.errorAlreadyExists(root, path, myReposRootDir));
        }
        /*
         * Check (recursively) to see if some lock is 'reserving' a path at that
         * location, or even some child-path; if so, check that we can use it.
         */
        if ((root.getTxnFlags() & FSConstants.SVN_FS_TXN_CHECK_LOCKS) != 0) {
            FSReader.allowLockedOperation(path, myAuthor, myLockTokens, true, false, myReposRootDir);
        }
     ...
     * node revision the new entry points to will be a directory, else it will
     * be a file. parent must be mutable, and must not have an entry named
     * entryName.
     */
    private FSRevisionNode makeEntry(FSRevisionNode parent, String parentPath, String entryName, boolean isDir, String txnId) throws SVNException {
        ...
        /*
         * We can safely call setEntry() because we already know that parent is
         * mutable, and we just created childNode, so we know it has no
         * ancestors (therefore, parent cannot be an ancestor of child)
         */
    ...
    /*
     * Get a new and unique to this transaction node-id for transaction txnId.
     */
    ...
    /*
     * Create a copy of fromPath in fromRoot named toPath in toRoot. If fromPath
    ...
    private void makeCopy(FSOldRoot fromRoot, String fromPath, FSOldRoot toRoot, String toPath, boolean preserveHistory) throws SVNException {
        ...
        /*
         * Build up the parent path from toPath in toRoot. If the last component
         * does not exist, it's not that big a deal. We'll just make one there.
         */
        ...
        /*
         * If the destination node already exists as the same node as the source
         * (in other words, this operation would result in nothing happening at
         * all), just do nothing an return successfully, proud that you saved
         * yourself from a tiresome task.
         */
        ...
        if (!fromRoot.isTxnRoot()) {
            ...
            /*
             * Copying from transaction roots not currently available. Note that
             * when copying from mutable trees, you have to make sure that you
             * aren't creating a cyclic graph filesystem, and a simple
             * referencing operation won't cut it. Currently, we should not be
             * able to reach this clause, and the interface reports that this
             * only works from immutable trees anyway, but this requirement need
    ...
    /*
     * Populating the 'changes' table. Add a change to the changes table in FS,
     * keyed on transaction id, and indicated that a change of kind "changeKind"
     * occurred on path (whose node revision id is - or was, in the case of a
     ...
     * copyFromPath specify under which revision and path the node was copied
     * from. If this was not part of a copy, copyFromrevision should be
     * FSConstants.SVN_INVALID_REVNUM.
     */
    ...
    public void changeDirProperty(String name, String value) throws SVNException {
        ...
        if (FSRepository.isValidRevision(dirBaton.getBaseRevision())) {
            /*
             * Subversion rule: propchanges can only happen on a directory which
             * is up-to-date.
             */
    ...
    /*
     * Change, add, or delete a node's property value. The node affect is path
     * under root, the property value to modify is propName, and propValue
    ...
    private void changeNodeProperty(FSOldRoot root, String path, String propName, String propValue) throws SVNException {
        ...
        FSParentPath parentPath = myRevNodesPool.getParentPath(root, path, true, myReposRootDir);
        /*
         * Check (non-recursively) to see if path is locked; if so, check that
         * we can use it.
         */
        if ((root.getTxnFlags() & FSConstants.SVN_FS_TXN_CHECK_LOCKS) != 0) {
    ...
    public void addFile(String path, String copyFromPath, long copyFromRevision) throws SVNException {
        ...
        if (copyFromPath != null && FSRepository.isInvalidRevision(copyFromRevision)) {
             ...
             * does -- that's because makeFile() already errors out if the file
             * already exists.
             */
    ...
    private void makeFile(FSOldRoot root, String path) throws SVNException {
        ...
        FSParentPath parentPath = myRevNodesPool.getParentPath(root, path, false, myReposRootDir);
        /*
         * If there's already a file by that name, complain. This also catches
         * the case of trying to make a file named `/'.
         */
        if (parentPath.getRevNode() != null) {
            SVNErrorManager.error(FSErrors.errorAlreadyExists(root, path, myReposRootDir));
        }
    ...
    public void openFile(String path, long revision) throws SVNException {
        ...
        /*
         * If the node our caller has is an older revision number than the one
         * in our transaction, return an out-of-dateness error.
         */
    ...
    public void applyTextDelta(String path, String baseChecksum) throws SVNException {
        ...
        if (baseChecksum != null) {
            /*
             * Until we finalize the node, its textRepresentation points to the
             * old contents, in other words, the base text.
             */
    ...
    /*
     * Make the node referred to by parentPath mutable, if it isn't already.
     * root must be the root from which parentPath descends. Clone any parent
    ...
    private void makePathMutable(FSOldRoot root, FSParentPath parentPath, String errorPath) throws SVNException {
        ...
        if (parentPath.getParent() != null) {
            ...
            if (root.isTxnRoot()) {
                ...
                /*
                 * One way or another, root_id now identifies a cloned root
                 * node.
                 */
    ...
    public SVNCommitInfo closeEdit() throws SVNException {
        /*
         * If no transaction has been created (i.e. if openRoot() wasn't called
         * before closeEdit()), abort the operation here with an error.
         */
        if (myTxn == null) {
    ...
    private long finalizeCommit() throws SVNException {
        ...
        FSHooks.runPreCommitHook(myReposRootDir, myTxn.getTxnId());
        /* Commit. */
    ...
    private long commitTxn() throws SVNException {
         ...
         * 
         * 1. A transaction, with a mutable tree hanging off it. 2. A base
         * revision, against which a txn tree was made. 3. A latest revision,
         * which may be newer than the base rev.
         * 
         * The problem is that if latest != base, then one can't simply attach
         * the txn root as the root of the new revision, because that would lose
         * all the changes between base and latest. It is also not acceptable to
         * insist that base == latest; in a busy repository, commits happen too
         * fast to insist that everyone keep their entire tree up-to-date at all
         * times. Non-overlapping changes should not interfere with each other.
         * 
         * The solution is to merge the changes between base and latest into the
         * txn tree [see the method merge()]. The txn tree is the only one of
         * the three trees that is mutable, so it has to be the one to adjust.
         * 
         * You might have to adjust it more than once, if a new latest revision
         * gets committed while you were merging in the previous one. For
         * example:
         * 
         * 1. Jane starts txn T, based at revision 6. 2. Someone commits (or
         * already committed) revision 7. 3. Jane's starts merging the changes
         * between 6 and 7 into T. 4. Meanwhile, someone commits revision 8. 5.
         * Jane finishes the 6-->7 merge. T could now be committed against a
         * latest revision of 7, if only that were still the latest.
         * Unfortunately, 8 is now the latest, so... 6. Jane starts merging the
         * changes between 7 and 8 into T. 7. Meanwhile, no one commits any new
         * revisions. Whew. 8. Jane commits T, creating revision 9, whose tree
         * is exactly T's tree, except immutable now.
         * 
         * Lather, rinse, repeat.
         */
        ...
        while (true) {
             ...
             * transaction. (We don't want the revisions table locked while we
             * do the main merge.) We call it "youngish" because new revisions
             ...
             * merge succeeds, this youngest root node will become the new base
             * root for the svn txn that was the target of the merge (but note
             * that the youngest rev may have changed by then).
             */
             ...
             * target's txn will become the same as youngishRootNode, so any
             * future merges will only be between that node and whatever the
             * root node of the youngest rev is by then.
             */
            ...
            synchronized (writeLock) {// multi-threaded synchronization within
                ...
                try {
                    ...
                    if (svne.getErrorMessage().getErrorCode() == SVNErrorCode.FS_TXN_OUT_OF_DATE) {
                        /*
                         * Did someone else finish committing a new revision
                         * while we were in mid-merge or mid-commit? If so,
                         * we'll need to loop again to merge the new changes in,
                         * then try to commit again. Or if that's not what
                         * happened, then just return the error.
                         */
                        long youngestRev = FSReader.getYoungestRevision(myReposRootDir);
    ...
    private long commit() throws SVNException {
        ...
        /*
         * Locks may have been added (or stolen) between the calling of previous
         * methods and commitTxn(), so we need to re-examine every changed-path
         * in the txn and re-verify all discovered locks.
         */
        verifyLocks();
        ...
        FSID newRootId = null;
        File revisionPrototypeFile = FSRepositoryUtil.getTxnRevFile(myTxn.getTxnId(), myReposRootDir);
        try {
            ...
            long changedPathOffset = FSWriter.writeFinalChangedPathInfo(protoFile, myTxnRoot, myReposRootDir);
        ...
        if (txnProps != null && !txnProps.isEmpty()) {
            if (txnProps.get(SVNProperty.TXN_CHECK_OUT_OF_DATENESS) != null) {
                FSWriter.setTransactionProperty(myReposRootDir, myTxn.getTxnId(), SVNProperty.TXN_CHECK_OUT_OF_DATENESS, null);
            }
            if (txnProps.get(SVNProperty.TXN_CHECK_LOCKS) != null) {
                FSWriter.setTransactionProperty(myReposRootDir, myTxn.getTxnId(), SVNProperty.TXN_CHECK_LOCKS, null);
            }
        }
        ...
        File txnPropsFile = FSRepositoryUtil.getTxnPropsFile(myTxn.getTxnId(), myReposRootDir);
        ...
        try {
            FSWriter.writeFinalCurrentFile(myTxn.getTxnId(), newRevision, startNodeId, startCopyId, myReposRootDir);
        } catch (IOException ioe) {
            SVNErrorMessage err = SVNErrorMessage.create(SVNErrorCode.IO_ERROR, ioe.getLocalizedMessage());
            SVNErrorManager.error(err, ioe);
        }
    ...
    private void verifyLocks() throws SVNException {
        ...
        /*
         * Now, traverse the array of changed paths, verify locks. Note that if
         * we need to do a recursive verification a path, we'll skip over
         * children of that path when we get to them.
         */
        ...
        for (int i = 0; i < changedPaths.length; i++) {
             ...
             * path? For an existing file or directory getting modified (text,
             * props), it means we hold the lock on the file or directory. For
             * paths being added or removed, we need to hold the locks for that
             * path and any children of that path.
             */
            ...
            /*
             * If we just did a recursive check, remember the path we checked
             * (so children can be skipped).
             */
            if (recurse) {
    ...
    /*
     * Merge changes between an ancestor and source node into txn. The ancestor
     * is either ancestorNode, or if that is null, txn's base node.
     * 
     * If the merge is successful, txn's base will become sourceNode, and its
     * root node will have a new ID, a successor of sourceNode.
     */
    private void mergeChanges(FSRevisionNode ancestorNode, FSRevisionNode sourceNode) throws SVNException {
        ...
        if (txnRootNode.getId().equals(ancestorNode.getId())) {
            /*
             * If no changes have been made in txn since its current base, then
             * it can't conflict with any changes since that base. So we just
             * set *both* its base and root to source, making txn in effect a
             * repeat of source. This would, of course, be a mighty silly thing
             * for the caller to do, and we might want to consider whether this
    ...
    private void merge(String targetPath, FSRevisionNode target, FSRevisionNode source, FSRevisionNode ancestor, String txnId) throws SVNException {
        ...
        /*
         * Base cases: Either no change made in source, or same change as made
         * in target. Both mean nothing to merge here.
         */
         ...
         * 
         * if (not all 3 are directories) { early exit with conflict; }
         *  // Property changes may only be made to up-to-date // directories,
         * because once the client commits the prop // change, it bumps the
         * directory's revision, and therefore // must be able to depend on
         * there being no other changes to // that directory in the repository.
         * if (target's property list differs from ancestor's) conflict;
         * 
         * For each entry NAME in the directory ANCESTOR:
         * 
         * Let ANCESTOR-ENTRY, SOURCE-ENTRY, and TARGET-ENTRY be the IDs of the
         * name within ANCESTOR, SOURCE, and TARGET respectively. (Possibly null
         * if NAME does not exist in SOURCE or TARGET.)
         * 
         * If ANCESTOR-ENTRY == SOURCE-ENTRY, then: No changes were made to this
         * entry while the transaction was in progress, so do nothing to the
         * target.
         * 
         * Else if ANCESTOR-ENTRY == TARGET-ENTRY, then: A change was made to
         * this entry while the transaction was in process, but the transaction
         * did not touch this entry. Replace TARGET-ENTRY with SOURCE-ENTRY.
         * 
         * Else: Changes were made to this entry both within the transaction and
         * to the repository while the transaction was in progress. They must be
         * merged or declared to be in conflict.
         * 
         * If SOURCE-ENTRY and TARGET-ENTRY are both null, that's a double
         * delete; flag a conflict.
         * 
         * If any of the three entries is of type file, declare a conflict.
         * 
         * If either SOURCE-ENTRY or TARGET-ENTRY is not a direct modification
         * of ANCESTOR-ENTRY (determine by comparing the node-id fields),
         * declare a conflict. A replacement is incompatible with a modification
         * or other replacement--even an identical replacement.
         * 
         * Direct modifications were made to the directory ANCESTOR-ENTRY in
         * both SOURCE and TARGET. Recursively merge these modifications.
         * 
        ...
        /*
         * Possible early merge failure: if target and ancestor have different
         * property lists, then the merge should fail. Propchanges can *only* be
         * committed on an up-to-date directory.
         */
        ...
        for (Iterator ancestorEntryNames = ancestorEntries.keySet().iterator(); ancestorEntryNames.hasNext();) {
            ...
            if (sourceEntry != null && ancestorEntry.getId().equals(sourceEntry.getId())) {
                 ...
                 * ANCESTOR-ENTRY in both SOURCE and TARGET. Recursively merge
                 * these modifications.
                 */
             ...
             * Remove it from sourceEntries, so it's easy later to loop over all
             * the source entries that didn't exist in ancestorEntries.
             */
    ...
    private void purgeTxn() {
        /*
         * Now, purge the transaction: remove the directory associated with this
         * transaction.
         */
